...
export default function UsersSecurityView({ onOpenSidebar }) {
  ...
  const [sessions, setSessions] = useState(() => window.sessionsData || INITIAL_SESSIONS_DATA);
  const [activities, setActivities] = useState(() => window.activitiesData || INITIAL_ACTIVITY_DATA);
  const [audits, setAudits] = useState(() => window.auditData || INITIAL_AUDIT_DATA);
  ...
  const defaultRolePermissions = useMemo(() => {
    ...
    roles.forEach(role => {
      perms[role.name] = {};
      const roleRbac = rbacMatrix.filter(r => r.rol_funcional_id === role.id);
      
      roleRbac.forEach(row => {
        const allowedActions = [];
        ALL_ACTIONS.forEach(action => {
          // the DB columns are named 'puede_ver', 'puede_crear', etc.
          const colName = `puede_${action.id}`;
          if (row[colName]) {
            allowedActions.push(action.id);
          }
        });
        // Store using modulo_sistema_id instead of string keys
        perms[role.name][row.modulo_sistema_id] = allowedActions;
      });
    });
    return perms;
  }, [roles, rbacMatrix]);
  ...
  useEffect(() => {
    ...
    if (userIdParam) {
      if (data.length > 0) {
        ...
        if (user) {
          // Hacemos el fetching del detalle completo para evitar que la pantalla 360 se vea vacía en algunos tabs
          ...
          // ID inválido, limpiar param para salir del estado de carga
  ...
  const filteredData = useMemo(() => {
    return data.filter(item => {
      ...
      const matchSearch = !search || 
                          ...
                          (item.document_number && item.document_number.includes(searchLower)) ||
                          item.login_identifiers?.some(id => id.identifier_value.toLowerCase().includes(searchLower));
  ...
  const handleExport = () => {
    showToast('Catálogo de usuarios exportado con éxito.');
  };
  ...
  const handleStartEdit360 = async (user, tab = 'resumen') => {
    ...
    const mapWizardData = (sourceUser) => ({
      ...
      companyId: sourceUser.companyId || sourceUser.empresa_id || '',
      role: sourceUser.role_name || sourceUser.role || sourceUser.rol || '',
      rol_id: sourceUser.role_id || sourceUser.rol_principal_id || '',
      tipo_usuario_id: sourceUser.tipo_usuario_id || '',
      user_type: sourceUser.user_type || userTypes.find(t => t.id == sourceUser.tipo_usuario_id)?.name || '',
  ...
  const handleSaveEdit360 = () => {
    ...
    if (!wizardData.first_name || !wizardData.first_name.trim()) {
      setEdit360Error('El nombre es obligatorio.');
      return;
    }
    if (!wizardData.last_name || !wizardData.last_name.trim()) {
      setEdit360Error('El apellido es obligatorio.');
      return;
    }
    if (!wizardData.companyId) {
      setEdit360Error('Debe seleccionar una empresa.');
      return;
    }
    if (!wizardData.role) {
      setEdit360Error('Debe seleccionar un rol.');
      return;
    }

    // Removed email/doc validations from here since they are handled in handleTriggerSaveEdit360

    // Removed scope validation

    // Map identifiers
    let updatedIdentifiers = [...(wizardData.login_identifiers || [])];
    const primIdx = updatedIdentifiers.findIndex(id => id.is_primary);
    const identifierType = wizardData.primary_access_type;
    const identifierValue = identifierType === 'EMAIL' ? wizardData.identificador_principal : wizardData.document_number;
    ...
    const finalUser = {
      ...wizardData,
      full_name: `${wizardData.first_name.trim()} ${wizardData.last_name.trim()}`,
  ...
  const handleTriggerSaveEdit360 = () => {
    ...
    if (wizardData.primary_access_type === 'EMAIL') {
      if (!wizardData.email || !emailRegex.test(wizardData.email)) {
        newErrors.email = 'Debe ingresar un correo válido para usar este método de acceso.';
      } else {
        ...
        if (emailTaken) {
          newErrors.email = 'Correo ya está en uso.';
        }
      }
    } else if (wizardData.primary_access_type === 'DOCUMENT') {
      if (!wizardData.document_number || !wizardData.document_number.trim()) {
        newErrors.document_number = 'Debe ingresar un documento para usar este método de acceso.';
      } else {
        ...
        if (docTaken) {
          newErrors.document_number = 'Documento ya registrado.';
        }
      }
    }
  ...
  const handleExecuteRevokeSingle = async () => {
    ...
    try {
      ...
      await usersService.revokeUserSession(sessionToRevoke.user_id, sessionToRevoke.id);
      fetchUserSessions(sessionToRevoke.user_id);
      addAuditLog(sessionToRevoke.user_id, 'Cierre de Sesión Manual', 'user_sessions', `ID Sesión: ${sessionToRevoke.id}`, 'Estado: Expirada', 'Revocación individual de sesión');
      addActivityLog(sessionToRevoke.user_id, 'Sesión Cerrada', `Sesión en ${sessionToRevoke.device} cerrada manualmente.`);
  ...
  useEffect(() => {
    if (activeTab360 === 'actividad' && detailUser) {
      fetchUserActivity(detailUser.id);
    }
  }, [activeTab360, detailUser]);
  ...
  useEffect(() => {
    if (activeTab360 === 'auditoria' && detailUser) {
  ...
  const renderDropdownItems = (item) => {
    ...
    // All accounts can view 360 detail
    items.push(
      <button 
        key="view-detail"
      ...
      >
        <Eye size={13} className="text-[var(--text-muted)]" /> Ver detalle 360
      </button>
    );
      ...
      if (actStatus === 'INVITATION_SENT' || actStatus === 'INVITATION_OPENED') {
        ...
        items.push(
          ...
          <button 
            key="change-role"
            className="w-full text-left px-4 py-1.5 text-[13px] text-[var(--text-primary)] font-semibold hover:bg-[var(--bg-color)] flex items-center gap-1.5" 
            onClick={() => handleStartEdit360(item, 'permisos')}
          >
            <Key size={13} className="text-[var(--text-muted)]" /> Cambiar rol
          </button>,
          <button 
            key="change-scope"
            className="w-full text-left px-4 py-1.5 text-[13px] text-[var(--text-primary)] font-semibold hover:bg-[var(--bg-color)] flex items-center gap-1.5" 
            onClick={() => handleStartEdit360(item, 'alcance')}
          >
            <SlidersHorizontal size={13} className="text-[var(--text-muted)]" /> Cambiar alcance
          </button>
        );
    ...
    items.push(
      <button 
        key="delete-user"
        className="w-full text-left px-4 py-1.5 text-[13px] text-rose-600 font-semibold hover:bg-[var(--bg-color)] flex items-center gap-1.5 border-t border-[var(--border-color)] pt-1.5 mt-1" 
        onClick={() => handleDeleteUserClick(item)}
      >
        <Trash2 size={13} className="text-rose-500" /> Eliminar cuenta
      </button>
    );
      ...
      <div className="flex-1 flex flex-col min-h-0 bg-[var(--bg-elevated)] border-b border-[var(--border-color)] animate-in fade-in duration-200">
        ...
        <div className="shrink-0 bg-[var(--bg-color)] p-6 border-b border-[var(--border-color)] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div>
              <div className="flex items-center gap-2.5 flex-wrap">
                ...
                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase border tracking-wider ${
                  detailUser.status === 'Activo' ? 'bg-emerald-50 text-emerald-700 border-emerald-100 dark:bg-emerald-500/10' : 'bg-rose-50 text-rose-700 border-rose-100 dark:bg-rose-500/10'
          ...
          <div className="flex items-center gap-2">
            {isEditing360 ? (
              <span className="px-3 py-1.5 text-xs font-black text-rose-600 bg-rose-50 border border-rose-200 rounded-lg dark:bg-rose-500/10 dark:text-rose-400 dark:border-rose-500/20 animate-pulse">
                Modo Edición Activo
              </span>
            ) : (
              <button 
                ...
                className="px-3 py-1.5 rounded-lg border border-[var(--border-color)] hover:bg-[var(--bg-color)] text-[12px] font-bold shadow-sm transition-all bg-rose-50 text-rose-600 border-rose-200 dark:bg-rose-500/10 dark:text-rose-400 dark:border-rose-500/20"
              >
                Editar
              </button>
            )}
            <button 
              onClick={() => {
                const userToReset = { ...detailUser };
                handleGoBack();
                handleResetPasswordDirect(userToReset);
              }}
              className="px-3 py-1.5 rounded-lg border border-[var(--border-color)] hover:bg-[var(--bg-color)] text-[12px] font-bold shadow-sm transition-all text-rose-500 border-rose-200"
              title="Restablecer contraseña / PIN"
            >
              Restablecer clave
            </button>
            <button 
              onClick={() => {
                const userToToggle = { ...detailUser };
                handleGoBack();
                handleToggleBlock(userToToggle);
              }}
              className="px-3 py-1.5 rounded-lg border border-[var(--border-color)] hover:bg-[var(--bg-color)] text-[12px] font-bold shadow-sm transition-all text-rose-500 border-rose-200"
            >
              {detailUser.status === 'Bloqueado' ? 'Desbloquear' : 'Bloquear'}
            </button>
        ...
        <div className="bg-[var(--bg-elevated)] px-6 border-b border-[var(--border-color)] flex items-center gap-5 text-[12px] font-bold text-[var(--text-muted)] select-none shrink-0 overflow-x-auto scrollbar-none">
          {[
            ...
            { id: 'alcance', label: 'Alcance Operativo' },
            ...
            <button 
              ...
              className={`py-3.5 border-b-2 font-bold tracking-wide transition-all ${
                activeTab360 === t.id ? 'border-rose-500 text-rose-600' : 'border-transparent hover:text-[var(--text-primary)]'
              }`}
            >
              {t.label}
            </button>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-[var(--bg-color)] p-4 rounded-xl border border-[var(--border-color)] space-y-3">
                      <h4 className="font-extrabold text-[var(--text-primary)] text-[12px] border-b border-[var(--border-color)] pb-1.5 flex items-center gap-1.5"><Users size={14} className="text-rose-500" /> Información Personal</h4>
                      ...
                      <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-[11.5px] items-center">
                        ...
                        <div className="flex flex-col w-full">
                          <input
                            ...
                            className={`px-2 py-1 text-[11px] rounded border bg-[var(--bg-elevated)] text-[var(--text-primary)] font-bold focus:outline-none w-full ${formErrors360.first_name ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-[var(--border-color)] focus:border-rose-500'}`}
                            placeholder="Nombre"
                          />
                        ...
                        <div className="flex flex-col w-full">
                          <input
                            ...
                            className={`px-2 py-1 text-[11px] rounded border bg-[var(--bg-elevated)] text-[var(--text-primary)] font-bold focus:outline-none w-full ${formErrors360.last_name ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-[var(--border-color)] focus:border-rose-500'}`}
                        ...
                        <div className="flex flex-col w-full">
                          <div className="flex gap-1.5 w-full">
                            <select
                              ...
                              className="px-2 py-1 text-[11px] rounded border border-[var(--border-color)] bg-[var(--bg-elevated)] text-[var(--text-primary)] font-bold focus:outline-none focus:border-rose-500"
                            >
                            ...
                            <input
                              ...
                              className={`px-2 py-1 text-[11px] rounded border bg-[var(--bg-elevated)] text-[var(--text-primary)] font-bold focus:outline-none flex-1 min-w-0 ${formErrors360.document_number ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-[var(--border-color)] focus:border-rose-500'}`}
                              placeholder="Número"
                            />
                          </div>
                        ...
                        <div className="flex flex-col w-full">
                          <input
                            ...
                            className={`px-2 py-1 text-[11px] rounded border bg-[var(--bg-elevated)] text-[var(--text-primary)] font-bold focus:outline-none w-full ${formErrors360.email ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-[var(--border-color)] focus:border-rose-500'}`}
                        ...
                        <select
                          ...
                          className="px-2 py-1 text-[11px] rounded border border-[var(--border-color)] bg-[var(--bg-elevated)] text-[var(--text-primary)] font-bold focus:outline-none focus:border-rose-500 w-full"
                        >
                    ...
                    <div className="bg-[var(--bg-color)] p-4 rounded-xl border border-[var(--border-color)] space-y-3">
                      <h4 className="font-extrabold text-[var(--text-primary)] text-[12px] border-b border-[var(--border-color)] pb-1.5 flex items-center gap-1.5"><Key size={14} className="text-rose-500" /> Parámetros de Acceso</h4>
                      <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-[11.5px] items-center">
                        ...
                        <div className="flex gap-4 items-center">
                          <label className="flex items-center gap-1.5 font-bold cursor-pointer select-none">
                            <input
                              ...
                              className="rounded text-rose-500 border-[var(--border-color)] focus:ring-rose-500 w-3.5 h-3.5"
                            />
                            Web
                          </label>
                          <label className="flex items-center gap-1.5 font-bold cursor-pointer select-none">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-[var(--bg-color)] p-4 rounded-xl border border-[var(--border-color)] space-y-3">
                      <h4 className="font-extrabold text-[var(--text-primary)] text-[12px] border-b border-[var(--border-color)] pb-1.5 flex items-center gap-1.5"><Building2 size={14} className="text-rose-500" /> Relación y Asignación</h4>
                      <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-[11.5px] items-center">
                        ...
                        <div className="flex flex-col w-full">
                          <select
                            ...
                            className={`px-2 py-1 text-[11px] rounded border bg-[var(--bg-elevated)] text-[var(--text-primary)] font-bold focus:outline-none w-full ${formErrors360.companyId ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-[var(--border-color)] focus:border-rose-500'}`}
                          >
                        ...
                        <div className="flex flex-col w-full">
                          <select value={wizardData.rol_id || ''} onChange={(e) => { const rId = e.target.value; handleChange('rol_id', rId); const rolObj = roles.find(r => r.id == rId); handleChange('role', rolObj ? rolObj.name : ''); }}
                            className={`px-2 py-1 text-[11px] rounded border bg-[var(--bg-elevated)] text-[var(--text-primary)] font-bold focus:outline-none w-full ${formErrors360.rol_id ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-[var(--border-color)] focus:border-rose-500'}`}
                          >
...
{roles.filter(r => r.id != wizardData.rol_id).map(r => (
  <label key={r.id} className="flex items-center gap-1.5 font-bold cursor-pointer select-none text-[11px]">
    ...
    {r.name}
                    ...
                    <div className="bg-[var(--bg-color)] p-4 rounded-xl border border-[var(--border-color)] space-y-3">
                      <h4 className="font-extrabold text-[var(--text-primary)] text-[12px] border-b border-[var(--border-color)] pb-1.5 flex items-center gap-1.5"><ShieldCheck size={14} className="text-rose-500" /> Seguridad e Inicios</h4>
                      <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-[11.5px] items-center">
                        ...
                        <div className="flex gap-2 items-center">
                          ...
                          {wizardData.mfaEnabled && (
                            <select
                              ...
                              className="px-2 py-0.5 text-[11px] rounded border border-[var(--border-color)] bg-[var(--bg-elevated)] text-[var(--text-primary)] font-bold focus:outline-none focus:border-rose-500"
                            >
                        ...
                        <div className="flex gap-2 items-center w-full">
                          <input
                            ...
                            className="px-2 py-1 text-[11px] rounded border border-[var(--border-color)] bg-[var(--bg-elevated)] text-[var(--text-primary)] font-bold focus:outline-none focus:border-rose-500 flex-1"
                          />
                          <button
                            type="button"
                        ...
                        <input
                          ...
                          className="px-2 py-1 text-[11px] rounded border border-[var(--border-color)] bg-[var(--bg-elevated)] text-[var(--text-primary)] font-bold focus:outline-none focus:border-rose-500 w-full font-mono"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                    <div className="bg-[var(--bg-color)] p-4 rounded-xl border border-[var(--border-color)] space-y-3">
                      <h4 className="font-extrabold text-[var(--text-primary)] text-[12px] border-b border-[var(--border-color)] pb-1.5 flex items-center gap-1.5"><Settings size={14} className="text-rose-500" /> Configuración Avanzada</h4>
                      <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-[11.5px] items-center">
                        <span className="text-[var(--text-muted)] font-semibold">Correo de Acceso:</span>
                        <input type="email" value={wizardData.correo_acceso || ''} onChange={(e) => handleChange('correo_acceso', e.target.value)} className="px-2 py-1 text-[11px] rounded border border-[var(--border-color)] bg-[var(--bg-elevated)] text-[var(--text-primary)] font-bold focus:outline-none focus:border-rose-500 w-full" placeholder="recovery@ejemplo.com" />
                        
                        <span className="text-[var(--text-muted)] font-semibold">Enviar Invitación:</span>
                        <label className="flex items-center gap-1.5 font-bold cursor-pointer select-none">
                          <input type="checkbox" checked={!!wizardData.enviar_invitacion_correo} onChange={(e) => handleChange('enviar_invitacion_correo', e.target.checked)} className="rounded text-rose-500 focus:ring-rose-500 w-3.5 h-3.5" /> Sí
                        </label>

                        <span className="text-[var(--text-muted)] font-semibold">Generar Clave Automática:</span>
                        <label className="flex items-center gap-1.5 font-bold cursor-pointer select-none">
                          <input type="checkbox" checked={!!wizardData.generar_clave_automatica} onChange={(e) => handleChange('generar_clave_automatica', e.target.checked)} className="rounded text-rose-500 focus:ring-rose-500 w-3.5 h-3.5" /> Sí
                        </label>

                        <span className="text-[var(--text-muted)] font-semibold">Forzar Cambio de Clave:</span>
                        <label className="flex items-center gap-1.5 font-bold cursor-pointer select-none">
                          <input type="checkbox" checked={!!wizardData.forzar_cambio_clave} onChange={(e) => handleChange('forzar_cambio_clave', e.target.checked)} className="rounded text-rose-500 focus:ring-rose-500 w-3.5 h-3.5" /> Sí
                        </label>

                        <span className="text-[var(--text-muted)] font-semibold">Idioma Preferido:</span>
                        <select value={wizardData.idioma_preferido || 'es'} onChange={(e) => handleChange('idioma_preferido', e.target.value)} className="px-2 py-1 text-[11px] rounded border border-[var(--border-color)] bg-[var(--bg-elevated)] text-[var(--text-primary)] font-bold focus:outline-none focus:border-rose-500 w-full">
                          <option value="es">Español</option>
                          <option value="en">Inglés</option>
                        </select>

                        <span className="text-[var(--text-muted)] font-semibold">Zona Horaria:</span>
                        <select value={wizardData.zona_horaria || 'America/Santo_Domingo'} onChange={(e) => handleChange('zona_horaria', e.target.value)} className="px-2 py-1 text-[11px] rounded border border-[var(--border-color)] bg-[var(--bg-elevated)] text-[var(--text-primary)] font-bold focus:outline-none focus:border-rose-500 w-full">
                        ...
                        <select value={wizardData.formato_fecha || 'DD/MM/YYYY'} onChange={(e) => handleChange('formato_fecha', e.target.value)} className="px-2 py-1 text-[11px] rounded border border-[var(--border-color)] bg-[var(--bg-elevated)] text-[var(--text-primary)] font-bold focus:outline-none focus:border-rose-500 w-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-[var(--bg-color)] p-4 rounded-xl border border-[var(--border-color)] space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-y-2 text-[11.5px]">
                      ...
                      <span className="text-[var(--text-muted)] font-semibold">Rol Asignado: <span className="text-red-500">*</span></span>
                      <span className="font-bold text-rose-500">{detailUser.role || detailUser.role_name || '—'}</span>
                        ...
                        <div className="relative border-l border-slate-200 dark:border-slate-800 ml-4 space-y-6 py-2">
                          {steps.map((step, idx) => {
                            ...
                            if (step.state === 'completed') {
                              ...
                              bulletColor = 'bg-rose-50 text-rose-600 border-rose-300 dark:bg-rose-500/10 dark:text-rose-400 dark:border-rose-500/20';
                          ...
                          if (actStatus === 'INVITATION_SENT' || actStatus === 'INVITATION_OPENED') {
                            buttons.push(
                              ...
                              <button 
                                ...
                                className="px-2.5 py-1 rounded bg-rose-500 text-white hover:bg-rose-600 text-[10.5px] font-bold"
                              >
                                Copiar Link
                              </button>
                            );
          ...
          {activeTab360 === 'permisos' && (
            <div className="space-y-4 animate-in fade-in duration-200">
                ...
                return (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
                    <div className="p-4 bg-[var(--bg-color)] border border-[var(--border-color)] rounded-xl space-y-1 shadow-xs">
                      ...
                      {isEditing360 && wizardData ? (
                        <select 
                          ...
                          className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded px-2 py-1.5 text-[12px] font-bold text-rose-500 focus:outline-none focus:border-rose-500 shadow-sm"
                        >
                        ...
                        <span className="text-sm font-black text-rose-500 block mt-1">{userRole}</span>
                      )}
                    </div>
                    <div className="p-4 bg-[var(--bg-color)] border border-[var(--border-color)] rounded-xl space-y-1 shadow-xs">
                          ...
                          <div className="absolute top-full left-0 w-full mt-1 bg-[var(--bg-elevated)] border border-[var(--border-color)] rounded-lg shadow-xl z-50 hidden group-hover:block max-h-[160px] overflow-y-auto custom-scrollbar">
                            {roles.filter(r => r.id != wizardData.rol_id).map(r => (
                               <label key={r.id} className="flex items-center gap-2 px-3 py-2 hover:bg-[var(--bg-color)] cursor-pointer border-b border-[var(--border-color)]/50 last:border-0 transition-colors">
                                  <input 
                                    type="checkbox" 
                                    className="rounded text-rose-500 focus:ring-rose-500 bg-[var(--bg-color)] border-[var(--border-color)] cursor-pointer"
                                    checked={(wizardData.roles_additional || []).includes(r.id)}
                                    onChange={() => {
                    ...
                    <div className="p-4 bg-[var(--bg-color)] border border-[var(--border-color)] rounded-xl space-y-1 shadow-xs col-span-1 sm:col-span-2 lg:col-span-1">
                      ...
                      <span className="text-[11px] font-bold text-rose-600 truncate block" title={criticalsStr}>
                        {criticalsStr}
                      </span>
                    </div>
                  </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between sm:items-center border-b border-[var(--border-color)] pb-2 gap-3">
                <div>
                  <h4 className="font-bold text-[var(--text-primary)] text-[12px]">Matriz de Permisos Efectivos</h4>
                  <p className="text-[10.5px] text-[var(--text-muted)] mt-0.5">Consulta de permisos consolidados (Heredados + Adicionales). No modificable directamente.</p>
                </div>
                <div className="flex flex-wrap items-center gap-2 text-[10px] font-bold select-none">
                  <span className="text-[10px] text-[var(--text-muted)] mr-2 uppercase tracking-wider">Filtrar:</span>
                  <button 
                    onClick={() => setMatrixFilter('all')}
                    className={`px-2.5 py-1.5 rounded-lg border transition-colors flex items-center gap-1.5 ${matrixFilter === 'all' ? 'bg-slate-800 text-white border-slate-800 dark:bg-slate-200 dark:text-slate-900' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300'}`}
                  >
                    Todos
                  </button>
                  <button 
                    onClick={() => setMatrixFilter('base')}
                    className={`px-2.5 py-1.5 rounded-lg border transition-colors flex items-center gap-1.5 ${matrixFilter === 'base' ? 'bg-rose-50 border-rose-200 text-rose-700 dark:bg-rose-500/20 dark:border-rose-500/30 dark:text-rose-300' : 'bg-white text-slate-600 border-slate-200 hover:bg-rose-50/50 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300'}`}
                  >
                    <span className={`w-3 h-3 rounded flex items-center justify-center border ${matrixFilter === 'base' ? 'bg-rose-500 text-white border-rose-600' : 'bg-rose-50 text-rose-500 border-rose-100 dark:bg-rose-500/10'}`}>✓</span> 
                    Rol Principal
                  </button>
                  <button 
                    onClick={() => setMatrixFilter('additional')}
                    className={`px-2.5 py-1.5 rounded-lg border transition-colors flex items-center gap-1.5 ${matrixFilter === 'additional' ? 'bg-amber-50 border-amber-200 text-amber-800 dark:bg-amber-500/20 dark:border-amber-500/30 dark:text-amber-300' : 'bg-white text-slate-600 border-slate-200 hover:bg-amber-50/50 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300'}`}
                  >
                    <span className={`w-3 h-3 rounded flex items-center justify-center border ${matrixFilter === 'additional' ? 'bg-amber-500 text-white border-amber-600' : 'bg-amber-100 text-amber-700 border-amber-300 dark:bg-amber-500/10'}`}>✓</span> 
                    Roles Adicionales
                  </button>
                </div>
              </div>

              <div className="overflow-x-auto border border-[var(--border-color)] rounded-xl bg-[var(--bg-elevated)] custom-scrollbar">
                <table className="w-full text-left border-collapse text-[11px]">
                  <thead>
                    <tr className="border-b border-[var(--border-color)] bg-slate-100 dark:bg-slate-800/50 font-black text-slate-700 dark:text-slate-300 text-[10px] uppercase">
                      <th className="py-3 px-4 tracking-wider">Módulo / Sección</th>
                      <th className="py-3 px-1 text-center w-14 tracking-wider text-rose-500">FULL</th>
                      {ALL_ACTIONS.map(act => <th key={act.id} className="py-3 px-1 text-center w-14 tracking-wider">{act.label}</th>)}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[var(--border-color)] bg-white dark:bg-transparent">
                    {modules.map(mod => {
                      const activeUser = (isEditing360 && wizardData) ? wizardData : detailUser;
                      const userRole = activeUser.role || activeUser.role_name || '—';
                      ...
                      return (
                        <tr key={mod.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/25 transition-colors">
                          <td className="py-2.5 px-4 font-bold text-slate-700 dark:text-slate-200">{mod.name}</td>
                          
                          {/* FULL Column (Read Only) */}
                          <td className="py-2.5 px-1 text-center select-none">
                            <div className="flex justify-center">
                              {allChecked ? (
                                <CheckSquare size={16} className="text-rose-500 bg-rose-50 rounded" />
                              ) : (
                          ...
                          {ALL_ACTIONS.map(act => {
                            ...
                            return (
                              <td key={act.id} className="py-2.5 px-1 text-center select-none">
                                <div className="flex justify-center">
                                  {isChecked ? (
                                    <div className={`${!isVisible ? 'opacity-30 grayscale' : ''}`} title={source === 'manual' ? "Permiso manual" : source === 'additional' ? "Rol Adicional" : "Rol Principal"}>
                                      {source === 'manual' ? (
                                        <CheckSquare size={16} className="text-blue-600 bg-blue-50 rounded border border-blue-200" />
                                      ) : source === 'additional' ? (
                                        <CheckSquare size={16} className="text-amber-500 bg-amber-50 rounded border border-amber-200" />
                                      ) : (
                                        <CheckSquare size={16} className="text-rose-500 bg-rose-50 rounded border border-rose-100" />
                                      )}
                                    </div>
                                  ) : (
                                    <Square size={16} className="text-slate-200 dark:text-slate-800" />
                                  )}
                                </div>
                              </td>
                            );
          ...
          {/* TAB ALCANCE OPERATIVO */}
          {activeTab360 === 'alcance' && (
            <div className="space-y-6 animate-in fade-in duration-200">
              {!isEditing360 ? (
                // VIEW MODE
                <div className="space-y-6">
                  <div className="bg-[var(--bg-color)] p-5 rounded-xl border border-[var(--border-color)] space-y-4">
                    <h4 className="font-extrabold text-[var(--text-primary)] text-[12.5px] border-b border-[var(--border-color)] pb-2 flex items-center gap-1.5">
                      <SlidersHorizontal size={14} className="text-rose-500" /> Resumen del Alcance Operativo
                    </h4>
                    <div className="grid grid-cols-2 gap-y-3 gap-x-4 text-[12px] max-w-md pt-1">
                      <span className="text-[var(--text-muted)] font-semibold">Empresa: <span className="text-red-500">*</span></span>
                      <span className="font-bold text-[var(--text-primary)]">
                        {companies.find(c => c.id == detailUser.companyId)?.name || 'LOTEKA'}
                      </span>
                      
                      <span className="text-[var(--text-muted)] font-semibold">Tipo de Alcance:</span>
                      <span className="font-bold text-rose-500">
                        {detailUser.scope_type === 'COMPANY' ? 'Toda la empresa' : 
                         detailUser.scope_type === 'GROUPING' ? 'Acceso por agrupación' :
                         detailUser.scope_type === 'ROUTE' ? 'Acceso por ruta operativa' :
                         detailUser.scope_type === 'AGENCY' ? 'Acceso por agencia' :
                         detailUser.scope_type === 'TERRITORY' ? 'Acceso por territorio' : 'Desconocido'}
                      </span>

                      <span className="text-[var(--text-muted)] font-semibold">Incluye subagrupaciones y agencias heredadas:</span>
                      <span className={`font-bold ${detailUser.include_children ? 'text-emerald-600' : 'text-[var(--text-muted)]'}`}>
                        {detailUser.include_children ? 'Sí' : 'No'}
                      </span>
                  ...
                  {detailUser.scope_type !== 'COMPANY' && (
                    <div className="bg-[var(--bg-color)] p-5 rounded-xl border border-[var(--border-color)] space-y-3">
                      <h5 className="font-bold text-[var(--text-primary)] text-[12px] border-b border-[var(--border-color)] pb-2">Elementos de Alcance Asignados</h5>
                      <div className="border border-[var(--border-color)] rounded-xl overflow-hidden bg-[var(--bg-elevated)] max-w-md">
                        <table className="w-full text-left border-collapse text-[11px]">
                          <thead>
                            <tr className="border-b border-[var(--border-color)] bg-[var(--bg-color)] font-bold text-[var(--text-muted)]">
                              <th className="py-2.5 px-3">Código</th>
                              <th className="py-2.5 px-3">Nombre</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-[var(--border-color)]">
                            {(!detailUser.scope_entity_ids || detailUser.scope_entity_ids.length === 0) ? (
                              <tr>
                                <td colSpan={2} className="py-3 px-3 text-center text-[var(--text-muted)] italic">No hay elementos explícitos asignados.</td>
                              </tr>
                            ) : (
                              detailUser.scope_entity_ids.map(id => {
                                let label = id;
                                let codigo = id;
                                if (detailUser.scope_type === 'AGENCY') {
                                  codigo = `AGENCIA (${id})`;
                                  const ag = agencias.find(a => a.id === id);
                                  if (ag) label = ag.name;
                                } else if (detailUser.scope_type === 'GROUPING') {
                                  codigo = `AGRUPACION (${id})`;
                                  const grp = agrupaciones.find(a => a.id === id);
                                  label = grp ? grp.name : `Agrupación #${id}`;
                                } else if (detailUser.scope_type === 'ROUTE') {
                                  codigo = `RUTA (${id})`;
                                  label = `Ruta #${id}`;
                                } else if (detailUser.scope_type === 'TERRITORY') {
                                  if (typeof id === 'string' && id.includes('_')) {
                                    const [type, valStr] = id.split('_');
                                    const val = Number(valStr);
                                    codigo = `${type} (${val})`;
                                    
                                    if (type === 'PAIS') label = paises.find(x => x.id === val)?.name || label;
                                    if (type === 'REGION') label = regiones.find(x => x.id === val)?.name || label;
                                    if (type === 'PROVINCIA') label = provincias.find(x => x.id === val)?.name || label;
                                    if (type === 'MUNICIPIO') label = municipios.find(x => x.id === val)?.name || label;
                                    if (type === 'DISTRITO') label = distritos.find(x => x.id === val)?.name || label;
                                    if (type === 'SECTOR') label = sectores.find(x => x.id === val)?.name || label;
                                  } else {
                                    codigo = `SECTOR (${id})`;
                                    const sec = sectores.find(s => s.id === Number(id));
                                    label = sec ? sec.name : `Sector #${id}`;
                                  }
                                }
                                return (
                                  <tr key={id} className="hover:bg-[var(--bg-color)]/50">
                                    <td className="py-2 px-3 font-mono font-bold text-rose-500">{codigo}</td>
                                    <td className="py-2 px-3 font-semibold text-[var(--text-primary)]">{label}</td>
                                  </tr>
                                );
                              })
                            )}
                  ...
                  {/* Permisos sobre el alcance card */}
                  <div className="bg-[var(--bg-color)] p-5 rounded-xl border border-[var(--border-color)] space-y-3">
                    <h5 className="font-bold text-[var(--text-primary)] mb-2.5 text-[12px] border-b border-[var(--border-color)] pb-1.5">Acciones Permitidas dentro del Alcance</h5>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      <div className={`p-3 rounded-lg border text-center font-bold text-[11px] ${detailUser.can_view ? 'bg-emerald-50/20 border-emerald-300 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/35' : 'bg-slate-50 border-[var(--border-color)] text-slate-400 dark:bg-slate-800/40 dark:text-slate-600'}`}>
                        Ver Analíticas: {detailUser.can_view ? 'Sí' : 'No'}
                      </div>
                      <div className={`p-3 rounded-lg border text-center font-bold text-[11px] ${detailUser.can_edit ? 'bg-emerald-50/20 border-emerald-300 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/35' : 'bg-slate-50 border-[var(--border-color)] text-slate-400 dark:bg-slate-800/40 dark:text-slate-600'}`}>
                        Editar Datos: {detailUser.can_edit ? 'Sí' : 'No'}
                      </div>
                      <div className={`p-3 rounded-lg border text-center font-bold text-[11px] ${detailUser.can_export ? 'bg-emerald-50/20 border-emerald-300 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/35' : 'bg-slate-50 border-[var(--border-color)] text-slate-400 dark:bg-slate-800/40 dark:text-slate-600'}`}>
                        Exportar Reportes: {detailUser.can_export ? 'Sí' : 'No'}
                      </div>
                      <div className={`p-3 rounded-lg border text-center font-bold text-[11px] ${detailUser.can_assign ? 'bg-emerald-50/20 border-emerald-300 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/35' : 'bg-slate-50 border-[var(--border-color)] text-slate-400 dark:bg-slate-800/40 dark:text-slate-600'}`}>
                        Asignar Agencias: {detailUser.can_assign ? 'Sí' : 'No'}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                // EDIT MODE
                <div className="space-y-6">
                  <div className="bg-[var(--bg-color)] p-5 rounded-xl border border-[var(--border-color)] space-y-4 animate-in slide-in-from-top-1.5 duration-200">
                    <h4 className="font-extrabold text-[var(--text-primary)] text-[12.5px] border-b border-[var(--border-color)] pb-2 flex items-center gap-1.5">
                      <SlidersHorizontal size={14} className="text-rose-500" /> Configuración de Alcance Operativo
                    </h4>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {/* Select Scope Type */}
                      <div>
                        <label className="block font-bold text-[var(--text-muted)] text-[11.5px] mb-1.5">Tipo de Alcance</label>
                        <select
                          value={wizardData.scope_type || 'COMPANY'}
                          onChange={(e) => {
                            const val = e.target.value;
                            handleChange('scope_type', val);
                            handleChange('scope_entity_ids', []);
                            if (val === 'COMPANY') {
                              handleChange('include_children', true);
                            }
                          }}
                          className="w-full bg-[var(--bg-elevated)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-[12.5px] font-bold text-[var(--text-primary)] focus:outline-none focus:border-rose-500 transition-colors"
                        >
                          <option value="COMPANY">Toda la Empresa (Alcance Global)</option>
                          <option value="GROUPING">Por agrupación comercial</option>
                          <option value="AGENCY">Por agencia específica</option>
                          <option value="TERRITORY">Por división geográfica (Territorio)</option>
                        </select>
                      </div>
                      ...
                      <div className="flex items-end pb-2">
                        <label className="flex items-center gap-2 text-[12px] font-semibold text-[var(--text-primary)] select-none cursor-pointer">
                          <input 
                            ...
                            className="w-4 h-4 rounded text-rose-500 focus:ring-rose-500 border-[var(--border-color)] bg-[var(--bg-elevated)]"
                          />
                          Incluye subagrupaciones y agencias heredadas
                        </label>
                      </div>
                    </div>
                    ...
                    {/* 2.1 Grouping checkboxes */}
                    {wizardData.scope_type === 'GROUPING' && (
                      <div className="space-y-2.5 p-4 bg-[var(--bg-elevated)] border border-[var(--border-color)] rounded-xl animate-in slide-in-from-top-1.5 duration-200">
                        <label className="block font-bold text-[var(--text-primary)] text-[12px]">Seleccionar Agrupaciones Comerciales</label>
                        <div className="grid grid-cols-2 gap-2 max-h-[140px] overflow-y-auto custom-scrollbar p-2 bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg">
                    ...
                    {/* 2.3 Agency selection panel */}
                    {wizardData.scope_type === 'AGENCY' && (
                      <div className="space-y-3 p-4 bg-[var(--bg-elevated)] border border-[var(--border-color)] rounded-xl animate-in slide-in-from-top-1.5 duration-200">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                          ...
                          <button
                            ...
                            className="flex items-center justify-center gap-1.5 px-3 py-1.5 text-[11px] font-bold bg-rose-500 hover:bg-rose-600 text-white rounded-lg transition-colors shadow-sm self-start sm:self-auto shrink-0"
                          >
                        ...
                        {/* Selected Agencies Summary list */}
                        <div className="p-3 bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg">
                          {(!wizardData.scope_entity_ids || wizardData.scope_entity_ids.length === 0) ? (
                            <div className="text-center py-4 text-[12px] text-[var(--text-muted)] italic">
                            ...
                            <div className="flex flex-wrap gap-2 max-h-[140px] overflow-y-auto custom-scrollbar pr-1">
                              {wizardData.scope_entity_ids.map(code => {
                                ...
                                return (
                                  <div
                                    key={code}
                                    className="flex items-center gap-1.5 pl-2.5 pr-1.5 py-1 bg-rose-500/10 border border-rose-500/25 rounded-lg text-[11px] font-bold text-rose-500"
                                  >
                      ...
                      <div className="space-y-3 p-4 bg-[var(--bg-elevated)] border border-[var(--border-color)] rounded-xl animate-in slide-in-from-top-1.5 duration-200">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                          <div>
                            ...
                            <select
                              ...
                              className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-1.5 text-[12.5px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500"
                            >
                          ...
                          <div>
                            ...
                            <select
                              ...
                              className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-1.5 text-[12.5px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500 disabled:opacity-50"
                            >
                          ...
                          <div>
                            <label className="block font-bold text-[var(--text-muted)] text-[11px] mb-1">Provincia</label>
                            <select
                        ...
                        <div className="flex justify-end mt-2">
                          <button
                            ...
                            disabled={!wizardData.scope_pais}
                            className="px-4 py-1.5 bg-rose-500 hover:bg-rose-600 text-white text-[12px] font-bold rounded-lg transition-colors shadow-sm disabled:opacity-50"
                          >
                            Añadir a la lista
                          </button>
                        </div>

                        {/* Selected Territories Summary list */}
                        <div className="mt-4 p-3 bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg">
                          {(!wizardData.scope_entity_ids || wizardData.scope_entity_ids.length === 0) ? (
                            <div className="text-center py-4 text-[12px] text-[var(--text-muted)] italic">
                              Ningún territorio seleccionado. Selecciona sectores en los filtros superiores para agregarlos a la lista.
                            </div>
                          ) : (
                            <div className="flex flex-wrap gap-2 max-h-[140px] overflow-y-auto custom-scrollbar pr-1">
                  ...
                  <div className="bg-[var(--bg-color)] p-5 rounded-xl border border-[var(--border-color)] space-y-3 animate-in slide-in-from-top-1.5 duration-200">
                    <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-[var(--bg-elevated)] border border-[var(--border-color)] p-4 rounded-xl">
                      ...
                      <label className={`flex items-center gap-2 cursor-pointer font-semibold text-[12px] ${!wizardData.can_view ? 'opacity-40 cursor-not-allowed' : ''}`}>
                        <input 
                          ...
                          className="rounded text-rose-500 disabled:opacity-40" 
                        />
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                      <div className="p-4 bg-[var(--bg-color)] border border-[var(--border-color)] rounded-xl space-y-1 shadow-xs">
                        ...
                        <span className="text-lg font-black text-rose-500 font-mono">{activeSessionsCount}</span>
                      </div>
                      <div className="p-4 bg-[var(--bg-color)] border border-[var(--border-color)] rounded-xl space-y-1 shadow-xs">
                    ...
                    <div className="flex justify-between items-center border-b border-[var(--border-color)] pb-2">
                      ...
                      <button 
                        ...
                        className="px-3 py-1 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 rounded text-[11px] font-bold transition-all dark:bg-rose-500/10 dark:text-rose-400 dark:border-rose-500/20"
                      >
                        Revocar todas las sesiones
                      </button>
                    </div>

                    <div className="border border-[var(--border-color)] rounded-xl overflow-hidden bg-[var(--bg-elevated)] shadow-sm">
                      <table className="w-full text-left border-collapse text-[11px]">
                        <thead>
                          <tr className="border-b border-[var(--border-color)] bg-slate-100 dark:bg-slate-800 font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider select-none">
                            <th className="py-2.5 px-3 cursor-pointer" onClick={() => handleSortSesiones('device')}>Dispositivo / Navegador {sortConfigSesiones.key === 'device' && (sortConfigSesiones.direction === 'asc' ? '↑' : '↓')}</th>
                            <th className="py-2.5 px-3 cursor-pointer" onClick={() => handleSortSesiones('ip')}>IP {sortConfigSesiones.key === 'ip' && (sortConfigSesiones.direction === 'asc' ? '↑' : '↓')}</th>
                            <th className="py-2.5 px-3 cursor-pointer" onClick={() => handleSortSesiones('location')}>Ubicación {sortConfigSesiones.key === 'location' && (sortConfigSesiones.direction === 'asc' ? '↑' : '↓')}</th>
                            <th className="py-2.5 px-3 cursor-pointer" onClick={() => handleSortSesiones('login_time')}>Inicio de sesión {sortConfigSesiones.key === 'login_time' && (sortConfigSesiones.direction === 'asc' ? '↑' : '↓')}</th>
                            <th className="py-2.5 px-3 cursor-pointer" onClick={() => handleSortSesiones('last_activity_at')}>Última actividad {sortConfigSesiones.key === 'last_activity_at' && (sortConfigSesiones.direction === 'asc' ? '↑' : '↓')}</th>
                            <th className="py-2.5 px-3 text-center cursor-pointer" onClick={() => handleSortSesiones('status')}>Estado {sortConfigSesiones.key === 'status' && (sortConfigSesiones.direction === 'asc' ? '↑' : '↓')}</th>
                            <th className="py-2.5 px-3 text-right pr-4">Acciones</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-[var(--border-color)]">
                          {(() => {
                            ...
                            return sorted.length === 0 ? (
                              <tr>
                                <td colSpan={7} className="py-4 px-3 text-center text-[var(--text-muted)] italic">No hay sesiones activas ni conexiones registradas.</td>
                              </tr>
                            ) : (
                              sorted.map(session => (
                                <tr key={session.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer">
                                <td className="py-2.5 px-3">
                                  <div className="flex flex-col">
                                    <span className="font-semibold text-[var(--text-primary)]">{session.device}</span>
                                  </div>
                                </td>
                                <td className="py-2.5 px-3 font-mono font-bold text-[var(--text-secondary)]">{session.ip}</td>
                                <td className="py-2.5 px-3 font-semibold text-[var(--text-secondary)]">{session.location || '—'}</td>
                                <td className="py-2.5 px-3 font-semibold text-[var(--text-secondary)]">{formatSafeDateTime(session.login_time)}</td>
                                <td className="py-2.5 px-3 font-semibold text-[var(--text-secondary)]">{formatSafeDateTime(session.last_activity_at)}</td>
                                <td className="py-2.5 px-3 text-center">
                                  <span className={`px-2 py-0.5 rounded text-[9.5px] font-bold uppercase ${
                                    session.status === 'ACTIVA' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100 dark:bg-emerald-500/10' : 'bg-slate-50 text-slate-600'
                                  }`}>{session.status}</span>
                                </td>
          ...
          {activeTab360 === 'actividad' && (
            <div className="space-y-4 animate-in fade-in duration-200">
              <h4 className="font-bold text-[var(--text-primary)] text-[12px] border-b border-[var(--border-color)] pb-2 flex items-center gap-1.5">
                <Clock size={14} className="text-rose-500" /> Registro de Actividad del Usuario
              </h4>
              ...
              {(() => {
                ...
                if (userActs.length === 0) {
                  return (
                    <div className="py-12 border border-dashed border-[var(--border-color)] rounded-xl bg-[var(--bg-elevated)] text-center text-[var(--text-muted)] space-y-2">
                      <Clock className="mx-auto text-slate-300 dark:text-slate-700 animate-pulse" size={32} />
                      <p className="font-bold text-[12px]">Este usuario aún no tiene actividad registrada.</p>
                      <p className="text-[11px] text-slate-400">Las acciones operativas ejecutadas por el usuario se registrarán en esta tabla.</p>
                    </div>
                  );
                }
                ...
                return (
                  <div className="border border-[var(--border-color)] rounded-xl overflow-hidden bg-[var(--bg-elevated)] shadow-sm">
                    <table className="w-full text-left border-collapse text-[11px]">
                      <thead>
                          <tr className="border-b border-[var(--border-color)] bg-slate-100 dark:bg-slate-800 font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider select-none">
                            <th className="py-2.5 px-3 cursor-pointer" onClick={() => handleSortActividad('timestamp')}>Fecha y Hora {sortConfigActividad.key === 'timestamp' && (sortConfigActividad.direction === 'asc' ? '↑' : '↓')}</th>
                            <th className="py-2.5 px-3 cursor-pointer" onClick={() => handleSortActividad('event')}>Evento {sortConfigActividad.key === 'event' && (sortConfigActividad.direction === 'asc' ? '↑' : '↓')}</th>
                            <th className="py-2.5 px-3 cursor-pointer" onClick={() => handleSortActividad('module')}>Módulo {sortConfigActividad.key === 'module' && (sortConfigActividad.direction === 'asc' ? '↑' : '↓')}</th>
                            <th className="py-2.5 px-3 cursor-pointer" onClick={() => handleSortActividad('result')}>Resultado {sortConfigActividad.key === 'result' && (sortConfigActividad.direction === 'asc' ? '↑' : '↓')}</th>
                            <th className="py-2.5 px-3 cursor-pointer" onClick={() => handleSortActividad('ip')}>IP {sortConfigActividad.key === 'ip' && (sortConfigActividad.direction === 'asc' ? '↑' : '↓')}</th>
                            <th className="py-2.5 px-3 cursor-pointer" onClick={() => handleSortActividad('device')}>Dispositivo {sortConfigActividad.key === 'device' && (sortConfigActividad.direction === 'asc' ? '↑' : '↓')}</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-[var(--border-color)]">
                          {(() => {
                            ...
                            return sortedActs.map(act => (
                              <tr key={act.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer">
                            <td className="py-2.5 px-3 font-semibold text-[var(--text-secondary)]">
                              {formatSafeDateTime(act.timestamp)}
                            </td>
                            <td className="py-2.5 px-3">
                              <div className="flex flex-col">
                                <span className="font-bold text-[var(--text-primary)]">{act.event || act.title || 'Actividad'}</span>
                                {act.desc && <span className="text-[10px] text-[var(--text-muted)] mt-0.5">{act.desc}</span>}
                              </div>
                            </td>
                            <td className="py-2.5 px-3 font-semibold text-[var(--text-secondary)]">{act.module || 'Seguridad'}</td>
                            <td className="py-2.5 px-3">
                              <span className={`px-2 py-0.5 rounded text-[9.5px] font-bold ${
                                (act.result || 'Exitoso') === 'Exitoso' || (act.result || 'Exitoso') === 'Éxito'
                                  ? 'bg-emerald-50 text-emerald-600 border border-emerald-100 dark:bg-emerald-500/10 dark:text-emerald-400' 
                                  : 'bg-rose-50 text-rose-600 border border-rose-100 dark:bg-rose-500/10'
                              }`}>
                                {act.result || 'Exitoso'}
                              </span>
                            </td>
                            <td className="py-2.5 px-3 font-mono font-bold text-[var(--text-secondary)]">{act.ip || act.ip_address || '—'}</td>
                            <td className="py-2.5 px-3 font-semibold text-[var(--text-secondary)]">{act.device || 'Navegador Web'}</td>
                          </tr>
                            ));
                          })()}
                      </tbody>
          ...
          {activeTab360 === 'auditoria' && (
            <div className="space-y-4 animate-in fade-in duration-200">
              <h4 className="font-bold text-[var(--text-primary)] text-[12px] border-b border-[var(--border-color)] pb-2 flex items-center gap-1.5">
                <FileText size={14} className="text-rose-500" /> Bitácora de Auditoría Administrativa (Audit Log)
              </h4>
              
              {isLoadingAudits ? (
                <div className="py-12 border border-dashed border-[var(--border-color)] rounded-xl bg-[var(--bg-elevated)] flex flex-col items-center justify-center space-y-3">
                  <div className="w-8 h-8 border-4 border-slate-200 border-t-rose-500 rounded-full animate-spin dark:border-slate-700 dark:border-t-rose-500"></div>
                  <p className="text-[12px] text-[var(--text-muted)] font-medium">Cargando bitácora de auditoría...</p>
                </div>
              ) : (() => {
                if (realUserAudits.length === 0) {
                  return (
                    <div className="py-12 border border-dashed border-[var(--border-color)] rounded-xl bg-[var(--bg-elevated)] text-center text-[var(--text-muted)] space-y-2">
                      <FileText className="mx-auto text-slate-300 dark:text-slate-700 animate-pulse" size={32} />
                      <p className="font-bold text-[12px]">No hay eventos administrativos registrados para este usuario.</p>
                      <p className="text-[11px] text-slate-400">Los cambios de rol, permisos, bloqueos y configuraciones administrativas se registrarán aquí.</p>
                    </div>
                  );
                }
                ...
                return (
                  <div className="border border-[var(--border-color)] rounded-xl overflow-hidden bg-[var(--bg-elevated)] shadow-sm">
                    <table className="w-full text-left border-collapse text-[11px]">
                      <thead>
                        <tr className="border-b border-[var(--border-color)] bg-slate-100 dark:bg-slate-800 font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider select-none">
                          <th className="py-2.5 px-3 cursor-pointer" onClick={() => handleSortAuditoria('performed_at')}>Fecha y Hora {sortConfigAuditoria.key === 'performed_at' && (sortConfigAuditoria.direction === 'asc' ? '↑' : '↓')}</th>
                          <th className="py-2.5 px-3 cursor-pointer" onClick={() => handleSortAuditoria('action')}>Acción {sortConfigAuditoria.key === 'action' && (sortConfigAuditoria.direction === 'asc' ? '↑' : '↓')}</th>
                          <th className="py-2.5 px-3 cursor-pointer" onClick={() => handleSortAuditoria('before_value')}>Cambio Realizado {sortConfigAuditoria.key === 'before_value' && (sortConfigAuditoria.direction === 'asc' ? '↑' : '↓')}</th>
                          <th className="py-2.5 px-3 cursor-pointer" onClick={() => handleSortAuditoria('performed_by')}>Ejecutado por {sortConfigAuditoria.key === 'performed_by' && (sortConfigAuditoria.direction === 'asc' ? '↑' : '↓')}</th>
                          <th className="py-2.5 px-3 cursor-pointer" onClick={() => handleSortAuditoria('reason')}>Motivo {sortConfigAuditoria.key === 'reason' && (sortConfigAuditoria.direction === 'asc' ? '↑' : '↓')}</th>
                          <th className="py-2.5 px-3 cursor-pointer" onClick={() => handleSortAuditoria('result')}>Resultado {sortConfigAuditoria.key === 'result' && (sortConfigAuditoria.direction === 'asc' ? '↑' : '↓')}</th>
                          <th className="py-2.5 px-3 cursor-pointer" onClick={() => handleSortAuditoria('ip')}>IP {sortConfigAuditoria.key === 'ip' && (sortConfigAuditoria.direction === 'asc' ? '↑' : '↓')}</th>
                          <th className="py-2.5 px-3 cursor-pointer" onClick={() => handleSortAuditoria('device')}>Dispositivo {sortConfigAuditoria.key === 'device' && (sortConfigAuditoria.direction === 'asc' ? '↑' : '↓')}</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-[var(--border-color)]">
                        {(() => {
                          ...
                          return sortedAudits.map(audit => (
                            <tr key={audit.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer">
                              <td className="py-2.5 px-3 font-semibold text-[var(--text-secondary)]">
                                {formatSafeDateTime(audit.performed_at || audit.timestamp)}
                              </td>
                              <td className="py-2.5 px-3 font-bold text-rose-600 uppercase tracking-wide">
                                {audit.action}
                              </td>
                              <td className="py-2.5 px-3 align-top">
                                <div className="flex flex-col gap-1.5 text-[10px] font-mono text-[var(--text-muted)] max-w-[280px]">
                                  <div>
                                    <span className="font-bold text-rose-500">Antes:</span>
                                    <div className="max-h-[70px] overflow-y-auto bg-[var(--bg-color)]/50 border border-[var(--border-color)]/50 p-1.5 rounded mt-0.5 whitespace-pre-wrap break-words custom-scrollbar shadow-inner text-[9px] leading-tight">
                                      {(() => {
                                        const val = audit.before_value || audit.before_state;
                                        if (!val || val === '—') return '—';
                                  ...
                                  <div>
                                    <span className="font-bold text-emerald-500">Después:</span>
                                    <div className="max-h-[70px] overflow-y-auto bg-[var(--bg-color)]/50 border border-[var(--border-color)]/50 p-1.5 rounded mt-0.5 whitespace-pre-wrap break-words custom-scrollbar shadow-inner text-[9px] leading-tight">
                                      {(() => {
                                        const val = audit.after_value || audit.after_state;
                                        if (!val || val === '—') return '—';
                                        try { return JSON.stringify(JSON.parse(val), null, 2); } 
                                        catch(e) { return val; }
                                      })()}
                              ...
                              <td className="py-2.5 px-3 font-medium text-amber-600">{audit.reason || 'Actualización administrativa'}</td>
                              <td className="py-2.5 px-3">
                                <span className={`px-2 py-0.5 rounded text-[9.5px] font-bold ${
                                  (audit.result || 'Completado') === 'Completado' || (audit.result || 'Completado') === 'Exitoso'
                                    ? 'bg-emerald-50 text-emerald-600 border border-emerald-100 dark:bg-emerald-500/10 dark:text-emerald-400' 
                                    : 'bg-rose-50 text-rose-600 border border-rose-100 dark:bg-rose-500/10'
                                }`}>
                              ...
                              <td className="py-2.5 px-3 font-mono text-[var(--text-secondary)]">{audit.ip || '—'}</td>
                              <td className="py-2.5 px-3 font-medium text-[var(--text-secondary)]">{audit.device || '—'}</td>
                            </tr>
                          ));
                        })()}
                      </tbody>
        ...
        {/* Ficha Footer */}
        <div className="p-4 md:p-6 border-t border-[var(--border-color)] bg-[var(--bg-elevated)] flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 shrink-0">
          <div className="flex-1">
            {edit360Error && (
              <div className="flex items-center gap-2 text-rose-650 dark:text-rose-450 text-[11px] font-bold animate-in fade-in duration-200">
            ...
            {isEditing360 ? (
              <>
                <button
                  ...
                  className="px-5 py-2 rounded-lg border border-[var(--border-color)] hover:bg-[var(--bg-color)] font-bold text-xs text-[var(--text-secondary)] transition-all"
                >
                  Cancelar Edición
                </button>
                <button
                  ...
                  className="px-6 py-2 rounded-lg bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs shadow-md transition-all flex items-center gap-1.5"
                >
                  <Save size={13} />
                  Guardar Cambios
                </button>
              ...
              <button 
                type="button"
                onClick={handleGoBack}
                className="px-6 py-2.5 rounded-lg border border-[var(--border-color)] hover:bg-[var(--bg-color)] font-bold shadow-sm text-xs"
              >
                Volver al Listado
              </button>
            )}
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-[var(--bg-color)] animate-in fade-in duration-200">
      ...
      {toast && (
        <div className="fixed top-20 right-6 z-[9999] animate-in slide-in-from-top-4 duration-300">
          <div className={`px-4 py-3 rounded-xl shadow-xl flex items-center gap-2.5 text-xs font-bold text-white ${toast.type === 'error' ? 'bg-rose-600' : 'bg-emerald-600'}`}>
      ...
      <div className="px-6 pt-6 pb-4 shrink-0 flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-[var(--border-color)] bg-[var(--bg-elevated)] shadow-sm">
        <div className="flex items-center gap-3">
          {onOpenSidebar && (
            <button 
              className="md:hidden p-1.5 rounded-lg bg-[var(--bg-color)] border border-[var(--border-color)] text-[var(--text-muted)] hover:text-rose-500 shadow-sm"
          ...
          <div>
            <h1 className="text-2xl font-black text-[var(--text-primary)] flex items-center gap-2">
              <Users className="text-rose-500 animate-pulse" size={24} />
              Usuarios
            </h1>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button 
            ...
            className="p-2 bg-[var(--bg-color)] text-[var(--text-primary)] border border-[var(--border-color)] hover:bg-[var(--border-color)] rounded-lg shadow-sm transition-colors" 
          ...
          <button 
            ...
            className="px-4 py-2 bg-rose-500 hover:bg-rose-600 text-white rounded-lg text-[13px] font-bold flex items-center gap-2 shadow-md transition-all"
          >
      ...
      <div className="px-6 py-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 xl:grid-cols-9 gap-3 shrink-0">
        ...
        <div className="bg-[var(--bg-elevated)] border border-[var(--border-color)] rounded-xl p-3 shadow-xs flex flex-col justify-between">
          <span className="text-[10px] font-bold text-rose-500 uppercase tracking-wider">Bloqueados</span>
          <span className="text-lg font-black text-rose-500 mt-1">{data.filter(u => u.status === 'Bloqueado').length}</span>
        </div>
        <div className="bg-[var(--bg-elevated)] border border-[var(--border-color)] rounded-xl p-3 shadow-xs flex flex-col justify-between">
        ...
        <div className="bg-[var(--bg-elevated)] border border-[var(--border-color)] rounded-xl p-3 shadow-xs flex flex-col justify-between">
          <span className="text-[10px] font-bold text-rose-500 uppercase tracking-wider">Inv. Expiradas</span>
          <span className="text-lg font-black text-rose-500 mt-1">{data.filter(u => u.activation?.activation_status === 'INVITATION_EXPIRED').length}</span>
        </div>
        <div className="bg-[var(--bg-elevated)] border border-[var(--border-color)] rounded-xl p-3 shadow-xs flex flex-col justify-between">
          <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Sin Último Acceso</span>
          <span className="text-lg font-black text-slate-650 mt-1">{data.filter(u => !u.last_login_at).length}</span>
        </div>
      </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative flex-1 min-w-[285px] max-w-sm">
            ...
            <input 
              ...
              className="w-full pl-9 pr-4 py-2 bg-[var(--bg-elevated)] border border-[var(--border-color)] rounded-lg text-[13px] font-medium focus:outline-none focus:border-rose-500 shadow-sm" 
              placeholder="Buscar por nombre, correo, usuario o documento..." 
            />
          </div>
          <button type="button" onClick={() => setIsFilterModalOpen(true)} className="px-4 py-2 flex items-center gap-2 border border-[var(--border-color)] rounded-lg text-[13px] font-bold bg-[var(--bg-elevated)] shadow-sm">
            <SlidersHorizontal size={14} /> Filtrar IAM {activeFiltersCount > 0 && <span className="bg-rose-500 text-white text-[10px] px-1.5 py-0.5 rounded-md">{activeFiltersCount}</span>}
          </button>
          ...
          <div className="flex items-center border border-[var(--border-color)] rounded-lg p-0.5 bg-[var(--bg-elevated)] shadow-sm shrink-0 ml-auto">
            <button 
              ...
              className={`p-1.5 rounded-md transition-colors ${viewMode === 'table' ? 'bg-rose-500 text-white' : 'text-[var(--text-muted)] hover:text-[var(--text-primary)]'}`}
            ...
            <button 
              ...
              className={`p-1.5 rounded-md transition-colors ${viewMode === 'grid' ? 'bg-rose-500 text-white' : 'text-[var(--text-muted)] hover:text-[var(--text-primary)]'}`}
          ...
          {activeFiltersCount > 0 && (
            <button onClick={handleClearFilters} className="text-xs font-bold text-rose-600 hover:underline">Limpiar filtros</button>
          )}
        </div>

        {/* Selected rows actions bar */}
        {selectedIds.length > 0 && (
          <div className="p-3 bg-rose-500/5 border border-rose-500/25 rounded-xl flex items-center justify-between animate-in slide-in-from-top-2">
                  ...
                  {sortedData.length === 0 ? (
                    ...
                    paginatedData.map(item => {
                      ...
                      return (
                        <tr 
                          key={item.id}
                          className={`hover:bg-[var(--bg-color)] transition-colors cursor-pointer text-[13px] ${isChecked ? 'bg-rose-500/5' : ''}`}
                          onClick={() => handleViewDetail(item, 'resumen')}
                        >
                          <td className="py-3 px-4 text-center" onClick={e => e.stopPropagation()}>
                            <input 
                              ...
                              className="rounded border-[var(--border-color)] text-rose-500 cursor-pointer"
                            />
                          </td>
                          
                          <td className="py-3 px-4">
                            <div className="flex items-center gap-3">
                              <div className="w-9 h-9 rounded-full bg-rose-500/10 text-rose-500 border border-rose-500/20 flex items-center justify-center font-bold text-xs shrink-0 shadow-xs">
            ...
            {sortedData.length === 0 ? (
              ...
              paginatedData.map(item => {
                ...
                return (
                  <div 
                    key={item.id} 
                    className={`bg-[var(--bg-elevated)] border rounded-2xl p-5 hover:border-rose-500/50 transition-all cursor-pointer relative flex flex-col justify-between min-h-[220px] shadow-xs group ${
                      isChecked ? 'border-rose-500 bg-rose-500/5' : 'border-[var(--border-color)]'
                    }`}
                    onClick={() => handleViewDetail(item, 'resumen')}
                  >
                    ...
                    <div className="flex items-start justify-between" onClick={e => e.stopPropagation()}>
                      <div className="flex items-center gap-2">
                        <input 
                          ...
                          checked={isChecked} 
                          onChange={(e) => toggleSelection(item.id, e)}
                          className="rounded border-[var(--border-color)] text-rose-500 cursor-pointer w-4 h-4"
                        />
                    ...
                    <div className="flex items-center gap-3 my-3">
                      <div className="w-10 h-10 rounded-full bg-rose-500/10 text-rose-500 border border-rose-500/20 flex items-center justify-center font-bold text-xs shrink-0 shadow-xs">
                        {item.full_name.split(' ').filter(Boolean).map(n => n[0]).join('').replace(/\./g, '').substring(0, 2).toUpperCase()}
                      </div>
          ...
          <div className="relative w-full max-w-lg bg-[var(--bg-elevated)] rounded-xl shadow-2xl flex flex-col border border-[var(--border-color)] max-h-[85vh] overflow-hidden animate-in zoom-in-95 duration-200">
             <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-color)] shrink-0 bg-[var(--bg-color)]">
                <h3 className="text-lg font-black text-[var(--text-primary)] flex items-center gap-2">
                  <Filter size={18} className="text-rose-500 animate-pulse" /> 
                  Filtros de Búsqueda
                </h3>
             ...
             <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar text-xs">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select value={companyFilter} onChange={(e) => setCompanyFilter(e.target.value)} className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500 shadow-sm">
                  ...
                  <div>
                    ...
                    <select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)} className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500 shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500 shadow-sm">
                  ...
                  <div>
                    ...
                    <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500 shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select value={mfaFilter} onChange={(e) => setMfaFilter(e.target.value)} className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500 shadow-sm">
                  ...
                  <div>
                    ...
                    <select value={lastAccessFilter} onChange={(e) => setLastAccessFilter(e.target.value)} className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500 shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select value={activationFilter} onChange={(e) => setActivationFilter(e.target.value)} className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500 shadow-sm cursor-pointer">
                      <option value="Todos">Todos los estados</option>
                  ...
                  <div>
                    ...
                    <select value={accessMethodFilter} onChange={(e) => setAccessMethodFilter(e.target.value)} className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500 shadow-sm cursor-pointer">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select value={invitationFilter} onChange={(e) => setInvitationFilter(e.target.value)} className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500 shadow-sm cursor-pointer">
                  ...
                  <div>
                    ...
                    <select value={firstLoginFilter} onChange={(e) => setFirstLoginFilter(e.target.value)} className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500 shadow-sm cursor-pointer">
                      <option value="Todos">Todos</option>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select 
                      ...
                      className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500 shadow-sm cursor-pointer"
                    >
                  ...
                  <div>
                    ...
                    <select 
                      ...
                      className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500 shadow-sm cursor-pointer disabled:opacity-55 disabled:cursor-not-allowed"
                    >
             ...
             <div className="p-4 md:p-6 border-t border-[var(--border-color)] bg-[var(--bg-color)] shrink-0 flex justify-between gap-3">
                ...
                <button onClick={() => setIsFilterModalOpen(false)} className="px-6 py-2.5 rounded-lg bg-rose-500 hover:bg-rose-600 text-white text-[13px] font-bold flex items-center justify-center flex-1 shadow-md">
                  Aplicar Filtros
                </button>
             </div>
          </div>
                          ...
                          >
                            <div 
                              className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 border text-[11px] font-black transition-all duration-300 ${
                                isActive 
                                  ? 'border-rose-500 bg-rose-500 text-white shadow-md shadow-rose-500/20 scale-105 ring-4 ring-rose-500/15' 
                                  : isCompleted 
              ...
              <div className="flex-1 flex flex-col min-w-0 overflow-hidden bg-[var(--bg-elevated)]">
                ...
                <div className="md:hidden bg-[var(--bg-color)] px-5 py-3 border-b border-[var(--border-color)] flex flex-col gap-2 shrink-0 select-none">
                  <div className="flex justify-between items-center text-[11px] font-bold text-[var(--text-muted)]">
                    ...
                    <span className="text-rose-500 tracking-wide font-black uppercase">
                      {[
                        'Identidad',
                        'Empresa y Rol',
                        'Acceso',
                ...
                <div className="space-y-6 animate-in fade-in duration-300">
                  ...
                  <div className="bg-[var(--bg-color)]/30 border border-[var(--border-color)] p-5 rounded-xl space-y-4 shadow-sm animate-in slide-in-from-top-1.5 duration-200">
                    <h5 className="font-extrabold text-[12px] text-[var(--text-primary)] flex items-center gap-2 border-b border-[var(--border-color)]/50 pb-2">
                      <div className="w-1 h-3 bg-rose-500/55 rounded-full"></div>
                      Datos Personales
                    </h5>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input 
                          ...
                          value={wizardData.first_name || ''} 
                          onChange={(e) => handleChange('first_name', e.target.value)} 
                          className={`w-full bg-[var(--bg-color)] border rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none ${fieldErrors.first_name ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-[var(--border-color)] focus:border-rose-500'}`} 
                          placeholder="Ej. Juan"
                        />
                      ...
                      <div>
                        ...
                        <input 
                          ...
                          value={wizardData.last_name || ''} 
                          onChange={(e) => handleChange('last_name', e.target.value)} 
                          className={`w-full bg-[var(--bg-color)] border rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none ${fieldErrors.last_name ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-[var(--border-color)] focus:border-rose-500'}`} 
                  ...
                  <div className="bg-[var(--bg-color)]/30 border border-[var(--border-color)] p-5 rounded-xl space-y-4 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        ...
                        <select 
                          value={wizardData.document_type || 'Cédula'} 
                          onChange={(e) => handleChange('document_type', e.target.value)} 
                          className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500 cursor-pointer"
                        >
                          <option value="Cédula">Cédula</option>
                          <option value="Pasaporte">Pasaporte</option>
                          <option value="RNC">RNC</option>
                      ...
                      <div>
                        ...
                        <input 
                          ...
                          className={`w-full bg-[var(--bg-color)] border rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none ${fieldErrors.document_number ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-[var(--border-color)] focus:border-rose-500'}`} 
                      ...
                      <div>
                        ...
                        <input 
                          ...
                          className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500" 
                    ...
                    <div>
                      ...
                      <input 
                        ...
                        value={wizardData.email || ''} 
                        onChange={(e) => handleChange('email', e.target.value)} 
                        className={`w-full bg-[var(--bg-color)] border rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none ${fieldErrors.email ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-[var(--border-color)] focus:border-rose-500'}`} 
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-[var(--border-color)]/50 pt-4">
                      ...
                      <div>
                        ...
                        <select
                          value={wizardData.companyId || ''}
                          onChange={(e) => handleChange('companyId', e.target.value)}
                          className={`w-full bg-[var(--bg-color)] border rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none ${fieldErrors.companyId ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-[var(--border-color)] focus:border-rose-500'}`}
                        >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                      <div>
                        ...
                        <select
                          ...
                          className={`w-full bg-[var(--bg-color)] border rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none ${fieldErrors.department_id ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-[var(--border-color)] focus:border-rose-500'}`}
                        >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          className={`w-full bg-[var(--bg-color)] border rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none ${fieldErrors.tipo_usuario_id ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-[var(--border-color)] focus:border-rose-500'}`}
                        >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <select
                          ...
                          className={`w-full bg-[var(--bg-color)] border rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none ${fieldErrors.rol_id ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-[var(--border-color)] focus:border-rose-500'}`}
                        >
                      ...
                      <div>
                        ...
                        <select
                          value={wizardData.primary_access_type || 'EMAIL'}
                          onChange={(e) => handleChange('primary_access_type', e.target.value)}
                          className={`w-full bg-[var(--bg-color)] border rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none ${fieldErrors.primary_access_type ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-[var(--border-color)] focus:border-rose-500'}`}
                        >
                          <option value="EMAIL">Correo electrónico</option>
                          <option value="DOCUMENT">Documento de identidad</option>
                        </select>
                        ...
                        <button
                          ...
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[12px] font-bold border transition-colors ${
                            (wizardData.roles_additional || []).includes(r.id)
                              ...
                              : 'bg-white border-[var(--border-color)] text-[var(--text-secondary)] hover:border-rose-300 hover:text-rose-600'
                          }`}
                        >
                ...
                <div className="space-y-6 animate-in fade-in duration-300">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                          ...
                          <select 
                            value={wizardData.document_type || 'Cédula'} 
                            onChange={(e) => handleChange('document_type', e.target.value)} 
                            className={`w-full bg-[var(--bg-color)] border rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none ${fieldErrors.document_type ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-[var(--border-color)] focus:border-rose-500'}`}
                          >
                            <option value="Cédula">Cédula</option>
                            <option value="Pasaporte">Pasaporte</option>
                            <option value="RNC">RNC</option>
                        ...
                        <div>
                          ...
                          <input 
                            ...
                            value={wizardData.phone || ''} 
                            onChange={(e) => handleChange('phone', e.target.value)} 
                            className={`w-full bg-[var(--bg-color)] border rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none ${fieldErrors.phone ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-[var(--border-color)] focus:border-rose-500'}`} 
                            placeholder="Ej. +1 (809) 555-0101"
                          />
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 animate-in fade-in zoom-in-95 duration-200">
                        <div>
                          ...
                          <input 
                            ...
                            className={`w-full bg-[var(--bg-color)] border rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none ${fieldErrors.password ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-[var(--border-color)] focus:border-rose-500'}`} 
                          />
                        ...
                        <div>
                          ...
                          <input 
                            ...
                            className={`w-full bg-[var(--bg-color)] border rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none ${fieldErrors.confirm_password ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-[var(--border-color)] focus:border-rose-500'}`} 
                          />
                    ...
                    <div className="flex flex-col gap-4 border-t border-[var(--border-color)]/50 pt-4">
                      {wizardData.primary_access_type === 'EMAIL' && (
                        <label className="flex items-center justify-between cursor-pointer group">
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-[var(--text-primary)] text-[12px] group-hover:text-rose-600 transition-colors">Enviar invitación por correo electrónico</span>
                          ...
                          <div className={`w-9 h-5 flex items-center rounded-full p-0.5 transition-colors ${wizardData.send_invitation ? 'bg-rose-500' : 'bg-gray-300'}`}>
                      ...
                      <label className="flex items-center justify-between cursor-pointer group">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-[var(--text-primary)] text-[12px] group-hover:text-rose-600 transition-colors">Generar contraseña automáticamente</span>
                        </div>
                        <div className={`w-9 h-5 flex items-center rounded-full p-0.5 transition-colors ${wizardData.auto_generate_password ? 'bg-rose-500' : 'bg-gray-300'}`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border border-[var(--border-color)] rounded-xl p-5 bg-[var(--bg-color)]/30 shadow-sm animate-in slide-in-from-top-1.5 duration-200" style={{animationDelay: '50ms'}}>
                    <div>
                      ...
                      <select value={wizardData.preferred_language || 'es'} onChange={(e) => handleChange('preferred_language', e.target.value)} className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500">
                    ...
                    <div>
                      ...
                      <select value={wizardData.timezone || 'America/Santo_Domingo'} onChange={(e) => handleChange('timezone', e.target.value)} className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500">
                    ...
                    <div>
                      ...
                      <select value={wizardData.date_format || 'DD/MM/YYYY'} onChange={(e) => handleChange('date_format', e.target.value)} className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500">
                        <option value="DD/MM/YYYY">DD/MM/YYYY</option>
                        <option value="MM/DD/YYYY">MM/DD/YYYY</option>
                        <option value="YYYY-MM-DD">YYYY-MM-DD</option>
                      </select>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      <button 
                        ...
                        className={`p-4 border rounded-xl text-left transition-all ${wizardData.scope_type === 'COMPANY' ? 'border-rose-500 bg-rose-50 shadow-sm ring-1 ring-rose-500' : 'border-[var(--border-color)] bg-[var(--bg-color)] hover:border-rose-300'}`}
                      >
                        ...
                        <p className={`text-[10.5px] leading-tight ${wizardData.scope_type === 'COMPANY' ? 'text-rose-600/80' : 'text-[var(--text-muted)]'}`}>Acceso total sin restricciones dentro de la empresa seleccionada.</p>
                      </button>
                      
                      <button 
                        type="button" 
                        onClick={() => { handleChange('scope_type', 'GROUPING'); handleChange('scope_entity_ids', []); }}
                        className={`p-4 border rounded-xl text-left transition-all ${wizardData.scope_type === 'GROUPING' ? 'border-rose-500 bg-rose-50 shadow-sm ring-1 ring-rose-500' : 'border-[var(--border-color)] bg-[var(--bg-color)] hover:border-rose-300'}`}
                      >
                        ...
                        <p className={`text-[10.5px] leading-tight ${wizardData.scope_type === 'GROUPING' ? 'text-rose-600/80' : 'text-[var(--text-muted)]'}`}>Restringido a zonas, rutas, grupos u otras agrupaciones comerciales.</p>
                      </button>

                      <button 
                        type="button" 
                        onClick={() => { handleChange('scope_type', 'AGENCY'); handleChange('scope_entity_ids', []); }}
                        className={`p-4 border rounded-xl text-left transition-all ${wizardData.scope_type === 'AGENCY' ? 'border-rose-500 bg-rose-50 shadow-sm ring-1 ring-rose-500' : 'border-[var(--border-color)] bg-[var(--bg-color)] hover:border-rose-300'}`}
                      >
                        <h6 className={`font-bold text-[12.5px] mb-1 ${wizardData.scope_type === 'AGENCY' ? 'text-rose-700' : 'text-[var(--text-primary)]'}`}>Por agencia</h6>
                        <p className={`text-[10.5px] leading-tight ${wizardData.scope_type === 'AGENCY' ? 'text-rose-600/80' : 'text-[var(--text-muted)]'}`}>Restringido a agencias específicas.</p>
                      </button>

                      <button 
                        type="button" 
                        onClick={() => { handleChange('scope_type', 'TERRITORY'); handleChange('scope_entity_ids', []); }}
                        className={`p-4 border rounded-xl text-left transition-all ${wizardData.scope_type === 'TERRITORY' ? 'border-rose-500 bg-rose-50 shadow-sm ring-1 ring-rose-500' : 'border-[var(--border-color)] bg-[var(--bg-color)] hover:border-rose-300'}`}
                      >
                        ...
                        <p className={`text-[10.5px] leading-tight ${wizardData.scope_type === 'TERRITORY' ? 'text-rose-600/80' : 'text-[var(--text-muted)]'}`}>Restringido por división geográfica.</p>
                      </button>
                    </div>
                          ...
                          return (
                            <label key={group.id} className="flex items-center gap-2 py-1.5 px-3 rounded hover:bg-[var(--bg-elevated)] cursor-pointer text-[12px]">
                              <input 
                                ...
                                className="rounded text-rose-500 w-3.5 h-3.5 focus:ring-rose-500 border-gray-300"
                              />
                              <span className="font-semibold">{group.name}</span>
                            </label>
                          );
                    ...
                    <div className="space-y-4 p-5 bg-[var(--bg-elevated)] border border-[var(--border-color)] rounded-xl animate-in fade-in duration-200 shadow-sm">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        ...
                        <button
                          ...
                          className="flex items-center justify-center gap-1.5 px-3.5 py-1.5 text-[11.5px] font-bold bg-rose-500 hover:bg-rose-600 text-white rounded-lg transition-colors shadow-sm self-start sm:self-auto shrink-0"
                        >
                          <SlidersHorizontal size={13} />
                          Gestionar Agencias
                        </button>
                          ...
                          <div className="flex flex-wrap gap-2 max-h-[200px] overflow-y-auto custom-scrollbar pr-1">
                            {wizardData.scope_entity_ids.map(code => {
                              ...
                              return (
                                <div
                                  key={code}
                                  className="flex items-center gap-1.5 pl-2.5 pr-1.5 py-1 bg-rose-50 border border-rose-200 rounded-lg text-[11px] font-bold text-rose-600"
                                >
                                  <span>{ag ? ag.name : code}</span>
                                  <span className="text-[9px] font-mono opacity-75">({ag ? ag.code : code})</span>
                                  <button
                    ...
                    <div className="space-y-4 p-5 bg-[var(--bg-elevated)] border border-[var(--border-color)] rounded-xl animate-in fade-in duration-200 shadow-sm">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                          ...
                          <select
                            ...
                            className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-1.5 text-[12px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500"
                          >
                            <option value="">-- Selecciona país --</option>
                            {paises.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                          </select>
                        ...
                        <div>
                          ...
                          <select
                            ...
                            className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-1.5 text-[12px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500 disabled:opacity-50"
                          >
                            <option value="">-- Todas las regiones --</option>
                            {regiones.filter(r => r.pais_id === Number(wizardData.scope_pais)).map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
                          </select>
                      ...
                      <div className="flex justify-end mt-2">
                        <button
                          ...
                          disabled={!wizardData.scope_pais}
                          className="px-4 py-1.5 bg-rose-500 hover:bg-rose-600 text-white text-[11.5px] font-bold rounded-lg transition-colors shadow-sm disabled:opacity-50"
                        >
                          Añadir a la lista
                        </button>
                      </div>
                ...
                <div className="space-y-6 animate-in fade-in duration-300">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <select value={wizardData.inactivity_timeout_minutes ?? 0} onChange={(e) => handleChange('inactivity_timeout_minutes', parseInt(e.target.value))} className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500">
                    ...
                    <div>
                      ...
                      <select value={wizardData.max_failed_attempts ?? 10} onChange={(e) => handleChange('max_failed_attempts', parseInt(e.target.value))} className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-[13px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-[var(--bg-color)] border border-[var(--border-color)] p-4 rounded-xl">
                    <label className="flex items-center gap-2 cursor-pointer font-semibold">
                      <input type="checkbox" checked={wizardData.require_export_approval || false} onChange={(e) => handleChange('require_export_approval', e.target.checked)} className="rounded text-rose-500" />
                      <span>Exigir aprobación para exportaciones de datos sensibles</span>
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer font-semibold">
                      <input type="checkbox" checked={wizardData.require_dual_validation || false} onChange={(e) => handleChange('require_dual_validation', e.target.checked)} className="rounded text-rose-500" />
                  ...
                  {(wizardData.require_dual_validation || wizardData.role === 'Administrador General') && (
                    <div className="flex items-center gap-1.5 text-[10.5px] font-bold text-rose-600 bg-rose-500/5 px-3 py-2 rounded-lg border border-rose-200/50">
              ...
              {currentStep === 6 && (
                <div className="space-y-4 animate-in fade-in duration-300">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-[var(--bg-color)] p-4 rounded-xl border border-[var(--border-color)] space-y-2.5">
                      <h5 className="font-extrabold text-[var(--text-primary)] text-[12px] flex items-center gap-2 border-b border-[var(--border-color)] pb-1.5">
                        <Users size={14} className="text-rose-500" /> Identidad
                      </h5>
                      <div className="grid grid-cols-2 gap-y-2 text-[11.5px]">
                        <span className="text-[var(--text-muted)] font-semibold">Nombre: <span className="text-red-500">*</span></span>
                    ...
                    <div className="bg-[var(--bg-color)] p-4 rounded-xl border border-[var(--border-color)] space-y-2.5">
                      <h5 className="font-extrabold text-[var(--text-primary)] text-[12px] flex items-center gap-2 border-b border-[var(--border-color)] pb-1.5">
                        <Key size={14} className="text-rose-500" /> Acceso
                      </h5>
                      <div className="grid grid-cols-2 gap-y-2 text-[11.5px]">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-[var(--bg-color)] p-4 rounded-xl border border-[var(--border-color)] space-y-2.5">
                      <h5 className="font-extrabold text-[var(--text-primary)] text-[12px] flex items-center gap-2 border-b border-[var(--border-color)] pb-1.5">
                        <Building2 size={14} className="text-rose-500" /> Empresa y Rol
                      </h5>
                      <div className="grid grid-cols-2 gap-y-2 text-[11.5px]">
                        <span className="text-[var(--text-muted)] font-semibold">Empresa: <span className="text-red-500">*</span></span>
                        <span className="font-bold text-[var(--text-primary)]">
                        ...
                        <span className="font-bold text-rose-500">{wizardData.role}</span>
                        <span className="text-[var(--text-muted)] font-semibold">Permisos:</span>
                        <span className="font-semibold">
                    ...
                    <div className="bg-[var(--bg-color)] p-4 rounded-xl border border-[var(--border-color)] space-y-2.5">
                      <h5 className="font-extrabold text-[var(--text-primary)] text-[12px] flex items-center gap-2 border-b border-[var(--border-color)] pb-1.5">
                        <SlidersHorizontal size={14} className="text-rose-500" /> Alcance Operativo
                      </h5>
                      <div className="grid grid-cols-2 gap-y-2 text-[11.5px]">
                  ...
                  <div className="bg-[var(--bg-color)] p-4 rounded-xl border border-[var(--border-color)] space-y-2.5">
                    <h5 className="font-extrabold text-[var(--text-primary)] text-[12px] flex items-center gap-2 border-b border-[var(--border-color)] pb-1.5">
                      <ShieldCheck size={14} className="text-rose-500" /> Seguridad
                    </h5>
              ...
              <div className="flex items-center gap-2">
                ...
                {currentStep < 6 ? (
                  <button 
                    ...
                    className="px-6 py-2.5 rounded-lg bg-rose-500 hover:bg-rose-650 text-white font-bold flex items-center gap-1 shadow-sm text-xs"
                  >
                  ...
                  <>
                    ...
                    {wizardData.primary_access_type === 'EMAIL' && wizardData.send_invitation ? (
                      <button 
                        ...
                        className="px-6 py-2.5 rounded-lg bg-rose-500 hover:bg-rose-650 text-white font-bold flex items-center gap-1 shadow-sm text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                      >
    ...
    document.body
            ...
            <form onSubmit={handleReasonSubmit} className="mt-4 space-y-4">
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg p-2.5 text-[12px] text-[var(--text-primary)] focus:outline-none focus:border-rose-500 min-h-[90px] resize-none"
                  required
                />
              </div>
              ...
              <div className="flex justify-end gap-2.5 border-t border-[var(--border-color)] pt-3 shrink-0">
                ...
                <button 
                  type="submit" 
                  className="px-5 py-2 bg-rose-500 hover:bg-rose-600 text-white rounded-lg text-xs font-bold shadow-sm"
                >
                  Confirmar Cambio
                </button>
              </div>
      ...
      {showPassModal && typeof document !== 'undefined' && createPortal(
        <div className="fixed inset-0 z-[999999] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-[var(--bg-elevated)] rounded-2xl shadow-2xl border border-[var(--border-color)] p-6 animate-in zoom-in-95 duration-200 text-center">
            ...
            <div className="my-5 p-3.5 bg-slate-100 dark:bg-slate-900 rounded-xl border border-[var(--border-color)] select-all font-mono font-black text-rose-500 text-[15px] tracking-widest">
              {tempPassword}
            </div>
            ...
            <button
              ...
              className="w-full py-2.5 bg-rose-500 hover:bg-rose-600 text-white text-xs font-bold rounded-xl shadow-md transition-all flex items-center justify-center gap-1.5"
            >
          ...
          <div className="relative w-full max-w-3xl bg-[var(--bg-elevated)] rounded-2xl shadow-2xl border border-[var(--border-color)] overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-color)] bg-[var(--bg-color)]/50">
              <div className="flex items-center gap-2.5">
                <div className="w-8 h-8 bg-rose-500/10 text-rose-500 border border-rose-500/20 rounded-lg flex items-center justify-center">
                  <Building2 size={16} />
                </div>
                <div>
            ...
            <div className="p-4 bg-[var(--bg-color)]/30 border-b border-[var(--border-color)] grid grid-cols-1 sm:grid-cols-2 gap-3 shrink-0">
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg pl-9 pr-3 py-1.5 text-[12px] font-semibold text-[var(--text-primary)] focus:outline-none focus:border-rose-500"
                />
              </div>
            ...
            <div className="px-6 py-2.5 bg-[var(--bg-color)]/10 border-b border-[var(--border-color)] flex flex-wrap items-center justify-between gap-3 shrink-0">
              <div className="text-[10px] text-[var(--text-secondary)] font-bold flex items-center gap-3">
                ...
                <span className="text-rose-500">Seleccionadas: {wizardData.scope_entity_ids?.length || 0}</span>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="px-2.5 py-1 text-[9.5px] font-bold text-rose-500 hover:bg-rose-500/10 border border-rose-500/20 hover:border-rose-500/30 rounded transition-all"
                >
                  Limpiar todo
                </button>
              </div>
                    ...
                    return (
                      <div
                        ...
                        className={`p-3.5 rounded-xl border text-left flex items-start gap-3.5 cursor-pointer transition-all ${
                          isSelected 
                            ? 'bg-rose-500/5 border-rose-500 shadow-sm ring-1 ring-rose-500/30' 
                            : 'bg-[var(--bg-elevated)] border-[var(--border-color)] hover:border-rose-400'
                        }`}
                      >
                        <input
                          type="checkbox"
                          ...
                          readOnly
                          className="rounded text-rose-500 focus:ring-0 mt-1 cursor-pointer"
                        />
            ...
            <div className="px-6 py-4 border-t border-[var(--border-color)] bg-[var(--bg-color)]/50 flex items-center justify-between shrink-0">
              <div className="flex flex-col">
                ...
                <span className="text-[9.5px] text-rose-500 font-bold mt-0.5">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  type="button"
                  onClick={() => setIsAgencyModalOpen(false)}
                  className="px-4 py-2 bg-rose-500 hover:bg-rose-600 text-white text-xs font-bold rounded-xl shadow-md transition-all flex items-center gap-1.5"
                >
      ...
      {showConfirmEditModal && typeof document !== 'undefined' && createPortal(
        <div className="fixed inset-0 z-[999999] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm transition-opacity" onClick={() => setShowConfirmEditModal(false)}></div>
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 text-center animate-in fade-in zoom-in duration-200">
             ...
             <h3 className="text-xl font-extrabold text-slate-800 mb-3 tracking-tight">¿Confirmar Guardado?</h3>
             ...
             <div className="flex items-center gap-3 w-full">
               <button onClick={() => setShowConfirmEditModal(false)} className="flex-1 py-3 px-4 rounded-xl bg-slate-100 text-slate-700 font-bold hover:bg-slate-200 transition-colors">
                 Cancelar
               </button>
               <button onClick={handleSaveEdit360} className="flex-1 py-3 px-4 rounded-xl bg-blue-500 text-white font-bold hover:bg-blue-600 transition-colors shadow-lg shadow-blue-500/30">
                 Confirmar
               </button>
             </div>
          </div>
      ...
      {showSuccessEditModal && typeof document !== 'undefined' && createPortal(
        <div className="fixed inset-0 z-[999999] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/20 transition-opacity" onClick={() => setShowSuccessEditModal(false)}></div>
          <div className="relative bg-[var(--bg-elevated)] w-full max-w-sm rounded-2xl shadow-2xl p-6 text-center border border-[var(--border-color)] animate-in fade-in zoom-in-95 duration-200">
             ...
             <button onClick={() => setShowSuccessEditModal(false)} className="w-full py-2.5 bg-emerald-500 hover:bg-emerald-600 active:bg-emerald-700 text-white rounded-xl text-sm font-bold shadow-md hover:shadow-lg transition-all">
               Entendido
             </button>
          </div>
        </div>,
      ...
      {showSuccessWizardModal && typeof document !== 'undefined' && createPortal(
        <div className="fixed inset-0 z-[999999] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/20 transition-opacity" onClick={() => {
          ...
          <div className="relative bg-[var(--bg-elevated)] w-full max-w-sm rounded-2xl shadow-2xl p-6 text-center border border-[var(--border-color)] animate-in fade-in zoom-in-95 duration-200">
             ...
             <button onClick={() => {
                ...
                setIsEditing(false);
                if (!showPassModal) setWizardData(null);
             }} className="w-full py-2.5 bg-emerald-500 hover:bg-emerald-600 active:bg-emerald-700 text-white rounded-xl text-sm font-bold shadow-md hover:shadow-lg transition-all">
               Entendido
             </button>
          </div>
        </div>,
        ...
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-[var(--bg-elevated)] rounded-2xl shadow-2xl flex flex-col border border-rose-200/50 overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 pt-8 pb-6 flex flex-col items-center text-center">
              <div className="w-16 h-16 rounded-full bg-rose-50 border border-rose-100 flex items-center justify-center mb-4 text-rose-500 dark:bg-rose-950/20 dark:border-rose-900/30 dark:text-rose-400">
                <ShieldX size={32} />
              </div>
            ...
            <div className="px-6 pb-6 pt-2 flex justify-center gap-3">
              ...
              <button 
                ...
                className="px-6 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-[13px] font-bold transition-colors flex items-center justify-center gap-2 min-w-[120px]"
              >
                Confirmar
              </button>
            </div>
      ...
      {showDeleteModal && typeof document !== 'undefined' && createPortal(
        <div className="fixed inset-0 z-[999999] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 text-center animate-in fade-in zoom-in duration-200">
             ...
             <h3 className="text-xl font-extrabold text-slate-800 mb-3 tracking-tight">¿Eliminar Cuenta?</h3>
             <p className="text-slate-500 text-sm mb-8 px-2 leading-relaxed font-medium">
             ...
             <div className="flex items-center gap-3 w-full">
               <button onClick={() => setShowDeleteModal(false)} className="flex-1 py-3 px-4 rounded-xl bg-slate-100 text-slate-700 font-bold hover:bg-slate-200 transition-colors">
                 Cancelar
               </button>
               <button onClick={handleConfirmDeleteUser} className="flex-1 py-3 px-4 rounded-xl bg-rose-500 text-white font-bold hover:bg-rose-600 transition-colors shadow-lg shadow-rose-500/30">
                 Sí, Eliminar
               </button>
             </div>
          </div>
