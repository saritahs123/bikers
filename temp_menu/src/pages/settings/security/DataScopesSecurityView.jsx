...
export default function DataScopesSecurityView({ onOpenSidebar }) {
  ...
  return (
    <div className="p-6 md:p-8 space-y-6 flex flex-col min-h-full animate-in fade-in duration-300">
      ...
      {/* Toast Alert */}
      {toast && (
        <div className="fixed bottom-6 right-6 z-50 bg-[var(--bg-elevated)] border border-rose-500/30 shadow-2xl p-4 rounded-xl flex items-center gap-3 animate-in slide-in-from-bottom-5">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-[var(--border-color)] pb-5 shrink-0">
        ...
        <div className="flex items-center gap-2.5 shrink-0 w-full md:w-auto">
          <button 
            ...
            className="w-full md:w-auto bg-rose-500 hover:bg-rose-600 text-white font-bold text-xs px-4 py-2.5 rounded-xl shadow-sm hover:shadow transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Save size={14} />
                ...
                return (
                  ...
                  >
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-[10px] shrink-0 ${
                      isSelected ? 'bg-rose-500 text-white' : 'bg-[var(--bg-color)] text-[var(--text-secondary)] border border-[var(--border-color)]'
                    }`}>
              ...
              <div className="flex items-center justify-between border-b border-[var(--border-color)] pb-3">
                <div>
                  <h3 className="font-black text-sm text-[var(--text-primary)]">
                    Parámetros de Alcance: <span className="text-rose-500">{selectedUser.full_name}</span>
                  </h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select 
                    ...
                    className="w-full bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-xs font-bold text-[var(--text-primary)] focus:outline-none focus:border-rose-500 cursor-pointer"
                  >
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          ...
                          className="flex items-center gap-1.5 px-3 py-1.5 text-[10.5px] font-bold bg-rose-500 hover:bg-rose-600 text-white rounded-lg transition-colors shadow-sm cursor-pointer"
                        >
                          ...
                          <div className="flex flex-wrap gap-2 max-h-[140px] overflow-y-auto custom-scrollbar pr-1">
                            {scopeState.scope_entity_ids.map(code => {
                              ...
                              return (
                                <div
                                  key={code}
                                  className="flex items-center gap-1.5 pl-2.5 pr-1.5 py-1 bg-rose-500/10 border border-rose-500/25 rounded-lg text-[10.5px] font-bold text-rose-500 animate-in fade-in zoom-in-95 duration-150"
                                >
                                  ...
                                  <button
                                    ...
                                    className="p-0.5 hover:bg-rose-500/20 rounded-md transition-colors cursor-pointer"
                                  >
                          ...
                          return (
                            <button
                              ...
                              className={`px-2.5 py-1 rounded-lg border text-[10.5px] font-semibold transition-all flex items-center gap-1.5 cursor-pointer ${
                                isAssigned 
                                  ? 'bg-rose-50 border-rose-300 text-rose-600 font-bold dark:bg-rose-500/10 dark:border-rose-500/20' 
                  ...
                  {visibleSimulatedRows.length === 0 ? (
                    ...
                    visibleSimulatedRows.map(row => (
                      <tr key={row.id} className="hover:bg-[var(--bg-color)]/30 font-medium">
                        <td className="py-2 px-3 font-bold text-rose-500">{row.id}</td>
          ...
          <div className="relative w-full max-w-3xl bg-[var(--bg-elevated)] rounded-2xl shadow-2xl border border-[var(--border-color)] overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
            ...
            <div className="px-6 py-2.5 bg-[var(--bg-color)]/10 border-b border-[var(--border-color)] flex flex-wrap items-center justify-between gap-3 shrink-0">
              <div className="text-[10px] text-[var(--text-secondary)] font-bold flex items-center gap-3">
                ...
                <span className="text-rose-500">Seleccionadas: {scopeState.scope_entity_ids?.length || 0}</span>
              </div>
