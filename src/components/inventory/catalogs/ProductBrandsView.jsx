...
export default function ProductBrandsView() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/inventario/marcas-producto");
      if (res.ok) {
        setPermissions({
        ...
        showToast("Error al cargar las marcas de producto.", "error");
      }
    } catch (err) {
      console.error("Error fetching marcas-producto:", err);
      showToast("Error de conexión al cargar marcas.", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const validateForm = () => {
    const errs = {};
    const codRes = validateRequiredText(formData.codigo, "El Código de la Marca", 50);
    if (!codRes.isValid) errs.codigo = codRes.message;

    const nomRes = validateRequiredText(formData.nombre, "El Nombre de la Marca", 100);
  ...
  const handleSave = async (e) => {
    ...
    try {
      const url = editingItem
        ? `/api/inventario/marcas-producto/${editingItem.id}`
        : "/api/inventario/marcas-producto";
      ...
      if (!res.ok) {
        showToast(json.message || "Error al guardar la marca.", "error");
        return;
      }

      showToast(editingItem ? "Marca actualizada exitosamente." : "Marca creada exitosamente.");
      setIsDrawerOpen(false);
      fetchData();
    } catch (err) {
      console.error("Error saving marca-producto:", err);
  ...
  const handleToggleStatus = async (item) => {
    try {
      ...
      const res = await fetch(`/api/inventario/marcas-producto/${item.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          codigo: item.codigo,
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/inventario/marcas-producto/${itemToDelete.id}`, {
        method: "DELETE"
      });
      const json = await res.json();
      if (res.ok) {
        showToast(json.message || "Marca eliminada exitosamente.");
        setIsDeletingModalOpen(false);
        setItemToDelete(null);
        fetchData();
      } else {
        showToast(json.message || "Error al eliminar marca.", "error");
        setIsDeletingModalOpen(false);
      }
    } catch (err) {
      console.error("Error deleting marca-producto:", err);
  ...
  const filteredData = data.filter((item) => {
    const matchesSearch =
      ...
      item.pais_origen?.toLowerCase().includes(search.toLowerCase()) ||
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-xl font-bold text-foreground">Marcas de Producto</h1>
              ...
              <p className="text-xs text-foreground-muted">
                Fabricantes y marcas comerciales de repuestos, accesorios y componentes
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-card border border-border rounded-2xl p-4 flex items-center justify-between">
          <div>
            <p className="text-xs text-foreground-muted">Total Marcas</p>
      ...
      <div className="bg-card border border-border rounded-2xl p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3 shadow-sm">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            placeholder="Buscar por código, nombre, país o descripción..."
      ...
      <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
        {loading ? (
          <div className="p-12 text-center text-xs text-foreground-muted flex flex-col items-center gap-3">
            ...
            <span>Cargando marcas de producto...</span>
          </div>
          ...
          <div className="p-12 text-center text-foreground-muted flex flex-col items-center gap-2">
            ...
            <p className="text-sm font-medium text-foreground">No hay marcas registradas.</p>
            <p className="text-xs">
              {search || statusFilter !== "Todos"
                ? "No se encontraron resultados para los filtros aplicados."
                : "Comienza registrando tu primera marca con el botón 'Nueva Marca'."}
            </p>
          </div>
        ) : (
                ...
                {paginatedData.map((item) => (
                  <tr key={item.id} className="hover:bg-surface/50 transition-colors">
                    ...
                    <td className="p-3.5 text-foreground-secondary">
                      {item.pais_origen ? (
                        <div className="flex items-center gap-1.5">
                          <Flag size={12} className="text-foreground-muted" />
                          <span>{item.pais_origen}</span>
                        </div>
                      ) : (
                        "—"
                      )}
                    ...
                    <td className="p-3.5 text-foreground-secondary">
                      {item.sitio_web ? (
                        ...
                        >
                          <Globe size={12} />
                          <span className="truncate max-w-[150px]">{item.sitio_web}</span>
                        </a>
                      ) : (
                        "—"
                      )}
                        ...
                        {permissions.puede_editar && (
                          <>
                            <button
                              ...
                              title={item.estado === "ACTIVO" ? "Inactivar marca" : "Activar marca"}
        ...
        {!loading && sortedData.length > 0 && (
          <div className="p-4 bg-surface border-t border-border flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-xs text-foreground-muted">
            <span>
              Mostrando {paginatedData.length} de {sortedData.length} marcas filtradas
            </span>
            ...
            <div className="w-screen max-w-md bg-card border-l border-border shadow-2xl flex flex-col font-sans">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="font-mono text-base font-bold text-foreground">
                      {editingItem ? "Editar Marca" : "Nueva Marca de Producto"}
                    </h2>
              ...
              <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-6 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ej. SHI, SRAM, FOX, MAX"
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ej. Shimano, SRAM, Fox Racing, Maxxis"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-foreground mb-1.5">País de Origen</label>
                    <input
                      type="text"
                      value={formData.pais_origen}
                      onChange={(e) => setFormData({ ...formData, pais_origen: e.target.value })}
                      placeholder="Ej. Japón, EE. UU., Taiwán"
                ...
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1.5">Sitio Web Oficial</label>
                  <input
                    type="url"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Notas o descripción general de la marca..."
              ...
              <div className="p-4 border-t border-border bg-surface flex items-center justify-end gap-2">
                ...
                >
                  ...
                  <span>{isSaving ? "Guardando..." : "Guardar Marca"}</span>
                </button>
              </div>
            </div>
          </div>
      ...
      <SecurityConfirmDialog
        ...
        variant="danger"
        title="¿Eliminar Marca?"
        description={`¿Está seguro de que desea eliminar la marca "${itemToDelete?.nombre || ''}"? Esta acción no se puede deshacer si no posee productos vinculados.`}
        confirmLabel="Eliminar"
        isLoading={isDeleting}
        loadingLabel="Eliminando..."
        details={itemToDelete ? [
          { label: 'Marca', value: itemToDelete.nombre },
