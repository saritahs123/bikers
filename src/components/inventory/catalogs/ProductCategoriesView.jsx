...
export default function ProductCategoriesView() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/inventario/categorias-producto");
      if (res.ok) {
        setPermissions({
        ...
        showToast("Error al cargar las categorías de producto.", "error");
      }
    } catch (err) {
      console.error("Error fetching categorias-producto:", err);
  ...
  const validateForm = () => {
    const errs = {};
    const codRes = validateRequiredText(formData.codigo, "El Código de la Categoría", 50);
    if (!codRes.isValid) errs.codigo = codRes.message;

    const nomRes = validateRequiredText(formData.nombre, "El Nombre de la Categoría", 100);
  ...
  const handleSave = async (e) => {
    ...
    try {
      const url = editingItem
        ? `/api/inventario/categorias-producto/${editingItem.id}`
        : "/api/inventario/categorias-producto";
      ...
      if (!res.ok) {
        showToast(json.message || "Error al guardar la categoría.", "error");
        return;
      }

      showToast(editingItem ? "Categoría actualizada exitosamente." : "Categoría creada exitosamente.");
      setIsDrawerOpen(false);
      fetchData();
    } catch (err) {
      console.error("Error saving categoria-producto:", err);
  ...
  const handleToggleStatus = async (item) => {
    try {
      ...
      const res = await fetch(`/api/inventario/categorias-producto/${item.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          codigo: item.codigo,
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/inventario/categorias-producto/${itemToDelete.id}`, {
        method: "DELETE"
      });
      const json = await res.json();
      if (res.ok) {
        showToast(json.message || "Categoría eliminada exitosamente.");
        setIsDeletingModalOpen(false);
        setItemToDelete(null);
        fetchData();
      } else {
        showToast(json.message || "Error al eliminar categoría.", "error");
        setIsDeletingModalOpen(false);
      }
    } catch (err) {
      console.error("Error deleting categoria-producto:", err);
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-xl font-bold text-foreground">Categorías de Producto</h1>
              ...
              <p className="text-xs text-foreground-muted">
                Familias de productos (Frenos, Transmisión, Ruedas, Lubricantes, etc.)
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-card border border-border rounded-2xl p-4 flex items-center justify-between">
          <div>
            <p className="text-xs text-foreground-muted">Total Categorías</p>
      ...
      <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
        {loading ? (
          <div className="p-12 text-center text-xs text-foreground-muted flex flex-col items-center gap-3">
            ...
            <span>Cargando categorías de producto...</span>
          </div>
        ) : paginatedData.length === 0 ? (
          <div className="p-12 text-center text-foreground-muted flex flex-col items-center gap-2">
            <Layers size={32} className="opacity-30" />
            <p className="text-sm font-medium text-foreground">No hay categorías registradas.</p>
            <p className="text-xs">
              {search || statusFilter !== "Todos"
                ? "No se encontraron resultados para los filtros aplicados."
                : "Comienza registrando tu primera categoría con el botón 'Nueva Categoría'."}
            </p>
          </div>
        ) : (
                        ...
                        {permissions.puede_editar && (
                          <>
                            <button
                              ...
                              title={item.estado === "ACTIVO" ? "Inactivar categoría" : "Activar categoría"}
        ...
        {!loading && sortedData.length > 0 && (
          <div className="p-4 bg-surface border-t border-border flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-xs text-foreground-muted">
            <span>
              Mostrando {paginatedData.length} de {sortedData.length} categorías filtradas
            </span>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="font-mono text-base font-bold text-foreground">
                      {editingItem ? "Editar Categoría" : "Nueva Categoría de Producto"}
                    </h2>
              ...
              <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-6 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ej. FRE, TRA, RUE, LUB"
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ej. Frenos, Transmisión, Neumáticos, Suspensiones"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Descripción de la categoría y productos que agrupa..."
      ...
      <SecurityConfirmDialog
        ...
        variant="danger"
        title="¿Eliminar Categoría?"
        description={`¿Está seguro de que desea eliminar la categoría "${itemToDelete?.nombre || ''}"? Esta acción no se puede deshacer si no posee productos vinculados.`}
        confirmLabel="Eliminar"
        isLoading={isDeleting}
        loadingLabel="Eliminando..."
        details={itemToDelete ? [
          { label: 'Categoría', value: itemToDelete.nombre },
