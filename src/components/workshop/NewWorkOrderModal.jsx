...
export default function NewWorkOrderModal({ isOpen, onClose, onSuccess }) {
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      const almacenesCatalog = catObj.almacenes || [];
      ...
      if (almacenesCatalog.length > 0) {
        setSelectedAlmacenId(String(almacenesCatalog[0].almacen_id));
      }
  ...
  const handleSubmit = async (e) => {
    ...
    const computedTotal = currentItems.reduce((acc, item) => {
    ...
    try {
      const payload = {
        ...
        presupuesto_estimado: parseFloat(computedTotal.toFixed(2)),
