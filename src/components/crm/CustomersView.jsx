...
// ============================================================================
// FORMATTERS & VALIDATORS (Dominican Cédula, RNC, and Phone)
// ============================================================================
            ...
            <div className="p-5 border-b border-border bg-surface flex items-start justify-between shrink-0">
              <div>
                ...
                <p className="text-xs text-foreground-muted mt-0.5 font-mono">
                  {editingItem ? "Modifique la información registrada del cliente." : "Complete el formulario para crear un nuevo cliente."}
                </p>
              </div>
            ...
            <form onSubmit={handleSave} className="p-6 overflow-y-auto flex-1 space-y-6 font-mono text-xs">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className={formData.tipo_cliente === "EMPRESA" ? "sm:col-span-2" : ""}>
                    <label className="block text-foreground-secondary mb-1 font-semibold">
                      {formData.tipo_cliente === "EMPRESA" ? "Razón Social / Nombre" : "Nombre"} <span className="text-error">*</span>
                    </label>
                    <input
                      type="text"
              ...
              <div className="space-y-4 pt-2 border-t border-border-subtle">
                ...
                <div className="pt-3 border-t border-border-subtle">
                  <label className="block text-foreground-secondary font-bold uppercase text-[11px] mb-2">
                    Canales de Contacto Autorizados
                  </label>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <label className="flex items-center gap-2.5 bg-surface border border-border rounded-xl p-3 cursor-pointer hover:border-primary/50 transition-colors">
                      <input
                        type="checkbox"
                        checked={Boolean(formData.contacto_whatsapp)}
                        onChange={(e) => setFormData({ ...formData, contacto_whatsapp: e.target.checked })}
                        className="rounded border-border text-primary focus:ring-0 bg-surface-elevated"
                      />
                      <span className="text-foreground text-xs font-semibold">Autoriza WhatsApp</span>
                    </label>
                    ...
                    <label className="flex items-center gap-2.5 bg-surface border border-border rounded-xl p-3 cursor-pointer hover:border-primary/50 transition-colors">
                      <input
                        ...
                        className="rounded border-border text-primary focus:ring-0 bg-surface-elevated"
                      />
                      <span className="text-foreground text-xs font-semibold">Autoriza Email</span>
                    </label>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2 pt-2 border-t border-border-subtle">
                <label className="block text-foreground-secondary font-semibold">Notas / Observaciones</label>
                <textarea
                  rows={3}
                  value={formData.notas}
                  onChange={(e) => setFormData({ ...formData, notas: e.target.value })}
                  placeholder="Detalles sobre las preferencias o historial del cliente..."
                  className="w-full bg-surface border border-border rounded-xl p-3 text-foreground focus:outline-none focus:border-primary"
                />
              </div>
            </form>

            {/* Footer */}
            <div className="p-4 border-t border-border bg-surface flex items-center justify-end gap-3 shrink-0 font-mono text-xs">
              <button
                type="button"
                onClick={() => setIsDrawerOpen(false)}
                className="px-4 py-2.5 rounded-xl border border-border bg-surface text-foreground-secondary hover:text-foreground hover:bg-hover transition-all cursor-pointer font-bold"
              >
                Cancelar
              </button>
              <button
                ...
                onClick={handleSave}
                disabled={isSaving}
                className="px-5 py-2.5 rounded-xl bg-primary-button-bg text-primary-foreground font-bold hover:brightness-110 disabled:opacity-50 transition-all flex items-center gap-2 cursor-pointer shadow-md"
              >
