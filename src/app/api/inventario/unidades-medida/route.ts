...
export async function GET() {
  try {
    ...
    if (!permsInv.puede_ver && !permsSeg.puede_ver) {
      return NextResponse.json({ error: "FORBIDDEN", message: "No tienes permisos para ver las unidades de medida." }, { status: 403 });
    }

    const rows = await query(`
      SELECT
        um.unidad_medida_id AS id,
        um.unidad_medida_id,
        um.codigo,
        um.nombre,
        um.descripcion,
        ...
        (
          ...
          WHERE p.unidad_medida_id = um.unidad_medida_id
        ) AS total_productos
      FROM admin.unidad_medida um
      ORDER BY um.nombre ASC
    `);

    const mapped = (rows || []).map((r: any) => ({
      id: r.unidad_medida_id,
    ...
    console.error("Error in GET /api/inventario/unidades-medida:", error);
    return NextResponse.json({ error: "SERVER_ERROR", message: "Error al obtener unidades de medida." }, { status: 500 });
  }
}

// POST /api/inventario/unidades-medida
  ...
  try {
    ...
    if (!permsSeg.puede_crear) {
      return NextResponse.json({
        error: "FORBIDDEN",
        message: "No posee permisos de administrador para crear unidades de medida."
      }, { status: 403 });
    }
    ...
    const permite_decimales = Boolean(body.permite_decimales);
    ...
    if (codigo.length > 20) {
      return NextResponse.json({ error: "VALIDATION_ERROR", message: "El código no puede exceder 20 caracteres." }, { status: 400 });
    }
    if (!nombre) {
    ...
    const dupCheck = await query(
      `SELECT codigo, nombre FROM admin.unidad_medida WHERE UPPER(codigo) = $1 OR UPPER(nombre) = $2 LIMIT 1`,
      [codigo, nombre.toUpperCase()]
    );
    if (dupCheck && dupCheck.length > 0) {
      const dup = dupCheck[0];
      if (dup.codigo.toUpperCase() === codigo) {
        return NextResponse.json({ error: "DUPLICATE_CODE", message: `Ya existe una unidad de medida con el código '${codigo}'.` }, { status: 409 });
      }
      return NextResponse.json({ error: "DUPLICATE_NAME", message: `Ya existe una unidad de medida con el nombre '${nombre}'.` }, { status: 409 });
    }

    const insertRes = await query(
      `INSERT INTO admin.unidad_medida (
         codigo, nombre, descripcion, permite_decimales, estado, fecha_registro, usuario_registro
       )
       VALUES ($1, $2, $3, $4, $5, NOW(), $6)
       RETURNING *`,
      [codigo, nombre, descripcion, permite_decimales, estado, session.usuario_id]
    );

    return NextResponse.json({ success: true, item: insertRes[0] }, { status: 201 });
  } catch (error: any) {
    console.error("Error in POST /api/inventario/unidades-medida:", error);
    return NextResponse.json({ error: "SERVER_ERROR", message: "Error al registrar unidad de medida." }, { status: 500 });
  }
}
