...
// GET /api/inventario/categorias-producto
export async function GET() {
  try {
    ...
    if (!permsInv.puede_ver && !permsSeg.puede_ver) {
      return NextResponse.json({ error: "FORBIDDEN", message: "No tienes permisos para ver las categorías de producto." }, { status: 403 });
    }

    const rows = await query(`
      SELECT
        cp.categoria_producto_id AS id,
        ...
        (
          ...
          WHERE p.categoria_producto_id = cp.categoria_producto_id
        ) AS total_productos
      FROM admin.categoria_producto cp
      ORDER BY cp.nombre ASC
    `);

    const mapped = (rows || []).map((r: any) => ({
      id: r.categoria_producto_id,
    ...
    console.error("Error in GET /api/inventario/categorias-producto:", error);
    return NextResponse.json({ error: "SERVER_ERROR", message: "Error al obtener categorías de producto." }, { status: 500 });
  }
}

// POST /api/inventario/categorias-producto
  ...
  try {
    ...
    if (!permsSeg.puede_crear) {
      return NextResponse.json({
        error: "FORBIDDEN",
        message: "No posee permisos de administrador para crear categorías de producto."
      }, { status: 403 });
    }
    ...
    const dupCheck = await query(
      `SELECT codigo, nombre FROM admin.categoria_producto WHERE UPPER(codigo) = $1 OR UPPER(nombre) = $2 LIMIT 1`,
      [codigo, nombre.toUpperCase()]
    );
    if (dupCheck && dupCheck.length > 0) {
      const dup = dupCheck[0];
      if (dup.codigo.toUpperCase() === codigo) {
        return NextResponse.json({ error: "DUPLICATE_CODE", message: `Ya existe una categoría con el código '${codigo}'.` }, { status: 409 });
      }
      return NextResponse.json({ error: "DUPLICATE_NAME", message: `Ya existe una categoría con el nombre '${nombre}'.` }, { status: 409 });
    }

    const insertRes = await query(
      `INSERT INTO admin.categoria_producto (
    ...
    console.error("Error in POST /api/inventario/categorias-producto:", error);
    return NextResponse.json({ error: "SERVER_ERROR", message: "Error al registrar categoría de producto." }, { status: 500 });
  }
}
