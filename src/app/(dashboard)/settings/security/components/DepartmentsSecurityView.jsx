...
export default function DepartmentsSecurityView() {
  ...
  const validateForm = (val = formData.nombre) => {
    const res = validateRequiredText(val, "El Nombre del Departamento", 100);
    if (!res.isValid) {
  ...
  const handleSave = async (e) => {
    ...
    if (!validateForm()) {
      showToast("El Nombre del Departamento es obligatorio.");
      return;
    }
    ...
    try {
      ...
      if (editingItem) {
        const res = await fetch(`/api/departamentos/${editingItem.id}`, {
          method: 'PUT',
        ...
        if (!res.ok) {
          ...
          throw new Error(errData.error || "Error al actualizar el departamento.");
        }
        ...
        if (!res.ok) {
          const errData = await res.json();
          throw new Error(errData.error || "Error al registrar el departamento.");
        }
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/departamentos/${itemToDelete.id}`, { method: 'DELETE' });
      if (!res.ok) {
        const errData = await res.json();
        throw new Error(errData.error || "Error al eliminar el departamento.");
      }
  ...
  const exportToExcel = () => {
    const headers = ["ID", "Nombre", "Estado", "Fecha de Creación", "Fecha de Actualización"];
    ...
    link.setAttribute("download", `Departamentos_${new Date().toISOString().substring(0, 10)}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);

    showToast("Archivo CSV/Excel de Departamentos exportado.");
  };

  // Filtered dataset (Searches across name and status)
    ...
    <div className="w-full space-y-6 font-sans text-[#e4e3d9] animate-in fade-in duration-300">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl md:text-3xl font-extrabold text-white tracking-tight">Departamentos</h1>
          <p className="text-sm text-slate-400 mt-1">Gestión administrativa de departamentos y áreas de la organización.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          <div>
            <p className="font-mono text-[11px] text-slate-400 uppercase tracking-widest font-bold">Total Departamentos</p>
            ...
            <div className="p-5 border-b border-[#2d3748] bg-[#0e1117] flex items-start justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  {editingItem ? "Modifica los datos del departamento seleccionado." : "Completa los campos para registrar un nuevo departamento."}
                </p>
              </div>
              <button 
                type="button" 
            ...
            <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-5 space-y-4 custom-scrollbar">
              ...
              <div className="space-y-1">
                ...
                <input 
                  ...
                  placeholder="Ej. Operaciones, Ventas, Contabilidad"
              ...
              <div className="p-3 bg-[#0e1117] border border-[#2d3748] rounded-xl flex items-center gap-2.5 mt-4">
                ...
                <p className="text-[11px] text-slate-300 leading-tight">
                  Asegúrese de que el nombre del departamento sea único en la organización.
                </p>
              </div>
            </form>
      ...
      {mounted && isDeletingModalOpen && itemToDelete && typeof document !== 'undefined' && createPortal(
        <div style={{ position: 'fixed', inset: 0, zIndex: 999999, display: 'flex', alignItems: 'center', justifyCenter: 'center', padding: '16px' }}>
          ...
          >
            ...
            <h3 className="text-lg font-bold text-white mb-2">¿Eliminar Departamento?</h3>
            <p className="text-xs text-slate-300 mb-6 leading-relaxed">
              ¿Está seguro que desea eliminar permanentemente el departamento <strong className="text-white">{itemToDelete?.nombre || ''}</strong>?
            </p>
