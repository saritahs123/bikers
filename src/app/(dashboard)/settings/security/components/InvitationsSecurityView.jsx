    ...
    if (status === 'FIRST_LOGIN_COMPLETED') {
      actionButtons.push(
        ...
        <button
          ...
          className="px-2.5 py-1.5 rounded-lg border border-[var(--border-color)] bg-[var(--bg-elevated)] hover:bg-[var(--bg-color)] text-primary font-bold text-[10.5px] flex items-center gap-1 transition-all"
        >
    ...
    <div className="p-6 pb-12 flex flex-col min-h-full space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3 shrink-0">
        {[
          ...
          { label: 'Expiradas', value: metrics.expiradas, sub: 'Superó 48h', color: 'border-primary/20 text-rose-455 bg-primary/5' },
          { label: 'Rebotadas', value: metrics.rebotadas, sub: 'Falló entrega', color: 'border-primary-fixed/30 text-primary-fixed bg-rose-700/5' }
        ].map((c, i) => (
        ...
        <div className="flex flex-col lg:flex-row gap-3 items-stretch lg:items-center">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="block w-full rounded-lg border-0 bg-[var(--bg-color)] py-2.5 pl-9 pr-8 text-[var(--text-primary)] ring-1 ring-inset ring-[var(--border-color)] placeholder:text-[var(--text-muted)] focus:ring-2 focus:ring-inset focus:ring-primary text-xs transition-all outline-none"
              value={searchText}
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              className={`px-3 py-2.5 rounded-lg border text-xs font-bold transition-all shadow-xs flex items-center gap-1.5 ${
                activeFiltersCount > 0 
                  ? 'border-rose-200 bg-rose-50 text-primary-fixed dark:bg-primary/10' 
            ...
            {activeFiltersCount > 0 && (
              <button
                ...
                className="px-3 py-2.5 rounded-lg border border-rose-200 text-primary text-xs font-bold hover:bg-rose-50 transition-all shadow-xs"
              >
                Limpiar Filtros
              </button>
            )}
            ...
            <div className="flex-1 overflow-y-auto py-4 space-y-4 custom-scrollbar text-xs">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <select
                    className="w-full p-2 rounded-lg bg-[var(--bg-color)] border border-[var(--border-color)] text-xs font-semibold focus:ring-1 focus:ring-primary outline-none"
                    value={filterCompany}
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3 px-4 sticky left-0 bg-[var(--bg-elevated)] group-hover:bg-[var(--bg-color)] transition-colors border-r border-[var(--border-color)] z-10 shadow-[2px_0_5px_rgba(0,0,0,0.02)]">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-primary/10 text-primary-fixed border border-primary/20 flex items-center justify-center font-bold text-[10.5px] shrink-0">
                            {item.full_name.split(' ').filter(Boolean).map(n => n[0]).join('').replace(/\./g, '').substring(0, 2).toUpperCase()}
                          </div>
                          <div className="overflow-hidden">
                            <span className="font-extrabold text-[var(--text-primary)] block truncate leading-tight group-hover:text-primary transition-colors">
                              {item.full_name}
                            </span>
                      ...
                      <td className="py-3 px-4 text-primary font-bold font-mono">
                        {act.invitation_expires_at ? (
                          new Date(act.invitation_expires_at) < new Date() ? (
                            <span className="text-primary-fixed font-black">Expirado</span>
                          ) : (
                    ...
                    <div className="relative border-l border-slate-200 dark:border-border ml-3.5 space-y-5 py-1">
                      {steps.map((st, i) => {
                        ...
                        if (st.state === 'completed') {
                          ...
                          bulletColor = 'bg-rose-50 text-primary-fixed border-rose-300 dark:bg-primary/10 dark:text-primary border-primary/20';
                      ...
                      <div key={index} className="p-2.5 rounded-lg bg-[var(--bg-color)] border border-[var(--border-color)] text-[11px] space-y-1">
                        ...
                        <div className="flex justify-between items-center text-[9px] text-[var(--text-muted)] pt-1 border-t border-[var(--border-color)] mt-1.5">
                          ...
                          <span className={`px-1 py-0.2 rounded font-bold uppercase text-[8px] ${
                            log.type === 'audit' ? 'bg-indigo-500/10 text-indigo-500' : 'bg-primary/10 text-primary'
                          }`}>
